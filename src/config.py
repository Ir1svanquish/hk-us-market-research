...
def load_secret_env_value(name: str) -> Optional[str]:
    """Load a secret from NAME, falling back to the protected NAME_FILE path."""
    ...
    try:
        ...
        logger.warning("%s_FILE could not be read: %s", name, exc)
        return None
    if not value:
        logger.warning("%s_FILE is empty", name)
        return None
    return value
...
class Config:
    ...
    finnhub_api_keys: List[str] = field(default_factory=list)
    ...
    # 港股专用开关；关闭时跳过慢速 Tushare 港股补充信息，仅保留快链路
    enable_hk_fundamental_context: bool = True
    # 基本面阶段总预算（秒）
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        finnhub_keys_str = os.getenv('FINNHUB_API_KEYS') or os.getenv('FINNHUB_API_KEY') or ''
        ...
        return cls(
            ...
            social_sentiment_api_key=load_secret_env_value('SOCIAL_SENTIMENT_API_KEY'),
            social_sentiment_api_url=os.getenv('SOCIAL_SENTIMENT_API_URL', 'https://api.adanos.org').rstrip('/'),
            finnhub_api_keys=finnhub_api_keys,
            finnhub_timeout_seconds=parse_env_float(
                os.getenv('FINNHUB_TIMEOUT_SECONDS'),
                8.0,
                field_name='FINNHUB_TIMEOUT_SECONDS',
                minimum=1.0,
            ),
            ...
            enable_hk_fundamental_context=os.getenv('ENABLE_HK_FUNDAMENTAL_CONTEXT', 'true').lower() == 'true',
    ...
    def _parse_llm_channels(cls, channels_str: str) -> List[Dict[str, Any]]:
        ...
        for raw_name in channels_str.split(','):
            ...
            # > LLM_{NAME}_API_KEY_FILE (single protected file).
            ...
            if not api_keys:
                ...
                if single_key:
                    api_keys = [single_key]
            if not api_keys:
                key_file_raw = os.getenv(f'LLM_{ch_upper}_API_KEY_FILE', '').strip()
                if key_file_raw:
                    ...
                    try:
                        ...
                        _logger.warning(
                            "LLM_%s_API_KEY_FILE could not be read: %s",
                            ch_upper,
                            exc,
                        )
                    else:
                        if file_key:
                            ...
                            _logger.warning("LLM_%s_API_KEY_FILE is empty", ch_upper)
    ...
    def _legacy_keys_to_model_list(
        ...
        # When using DeepSeek through OPENAI_API_KEY + OPENAI_BASE_URL=https://api.deepseek.com,
        # do not auto-build __legacy_openai__ router deployments for deepseek/* models.
        # That path makes LiteLLM hit /beta/chat/completions and can trigger
        # `Authentication Fails (governor)` even though direct OpenAI-compatible
        # calls to https://api.deepseek.com/chat/completions succeed.
        if openai_base_url and 'api.deepseek.com' in openai_base_url:
            model_list = [entry for entry in model_list if entry.get('model_name') != '__legacy_openai__']
...
def extra_litellm_params(model: str, config: Config) -> Dict[str, Any]:
    ...
    """
    params: Dict[str, Any] = {}
    # deepseek/ provider: when users configure DeepSeek via OPENAI-compatible
    # env vars (OPENAI_API_KEY + OPENAI_BASE_URL=https://api.deepseek.com),
    # pass api_base explicitly to avoid LiteLLM falling back to beta routes.
    if model.startswith("deepseek/"):
        if config.openai_base_url and 'api.deepseek.com' in config.openai_base_url:
