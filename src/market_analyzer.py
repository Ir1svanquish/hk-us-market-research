...
class MarketOverview:
    ...
    # US AI 拥挤度监控（仅美股使用）
    ai_bubble_monitor: Optional[Dict[str, Any]] = None
...
"""
            if self.region == "us":
                return """### 6. Strategy Framework
- **Trend Structure**: Determine whether the market is in an uptrend, range, or defensive phase.
- **Liquidity & Sentiment**: Track breadth, volatility, and whether leadership is narrowing.
...
"""
        if self.region == "us":
            return """### 6. 策略框架
- **趋势结构**：先判断美股当前属于上升趋势、区间震荡，还是防御/风险回避阶段。
- **流动性与情绪**：重点跟踪指数广度、波动率变化，以及龙头是否开始收窄或扩散。
- **主线方向**：聚焦有催化剂且趋势延续的主线板块，同时回避高位扩散后的普跌风险。
"""
        if self.region == "hk":
            return """### 6. 策略框架
- **趋势结构**：根据恒指、恒生科技、国企指数的同步性判断市场属于趋势、多空拉锯还是风险回避。
- **资金与情绪**：重点观察南向资金方向、宏观叙事变化与风险偏好切换。
- **主线方向**：聚焦科技互联网主线延续性，以及金融、地产、政策链条的敏感变化。
"""
    ...
    def _resolve_market_review_date(self) -> str:
        """按市场交易日语义确定复盘标题日期。"""
        now_utc = datetime.now(ZoneInfo("UTC"))
        if self.region == "us":
            ny_now = now_utc.astimezone(ZoneInfo("America/New_York"))
            review_day = ny_now.date()
            # 收盘后 30 分钟的复盘常发生在北京时间次日凌晨；标题仍应归属美东交易日
            if ny_now.hour < 12:
                review_day = review_day - timedelta(days=1)
            return review_day.strftime('%Y-%m-%d')
        if self.region == "hk":
            hk_now = now_utc.astimezone(ZoneInfo("Asia/Hong_Kong"))
            return hk_now.date().strftime('%Y-%m-%d')
    ...
    def get_market_overview(self) -> MarketOverview:
        ...
        """
        today = self._resolve_market_review_date()
        ...
        # 5. 美股 AI 拥挤度监控（仅 US）
        if self.region == "us":
            overview.ai_bubble_monitor = self._build_ai_bubble_monitor(overview)
    ...
    def _inject_data_into_review(
        ...
        ai_bubble_block = self._build_ai_bubble_block(overview)
        if self.region == "us" and ai_bubble_block and "AI拥挤度监控" not in review:
            review = self._insert_before_risk_section(review, ai_bubble_block)
    ...
    @staticmethod
    def _insert_before_risk_section(text: str, block: str) -> str:
        import re
        match = re.search(r'\n###\s*七、风险提示', text)
        if match:
    ...
    def _build_ai_bubble_monitor(self, overview: MarketOverview) -> Dict[str, Any]:
        """构建美股 AI 拥挤度监控，复用独立原型服务。"""
        monitor = {
            "status": "数据不足",
            "phase": "等待更多样本",
            "structure": "结构信号尚未统一",
            "strategy": "维持现有主线跟踪，等待更多确认信号。",
            "risk_lines": [],
            "signals": [],
        }

        try:
            result = AIBubbleMonitorService(data_manager=self.data_manager).run()
            signals = []
            leader = (result.layers or {}).get('leaders', {})
            platform = (result.layers or {}).get('platforms', {})
            high_beta = (result.layers or {}).get('high_beta', {})
            if leader.get('avg_return_5d') is not None:
                signals.append(f"龙头层5日均涨幅 {leader['avg_return_5d']:.2f}%")
            if platform.get('avg_return_5d') is not None:
                signals.append(f"平台层5日均涨幅 {platform['avg_return_5d']:.2f}%")
            if high_beta.get('avg_return_5d') is not None:
                signals.append(f"高弹性层5日均涨幅 {high_beta['avg_return_5d']:.2f}%")
            if leader.get('social_heat_avg') is not None and high_beta.get('social_heat_avg') is not None:
                signals.append(f"龙头热度 {leader['social_heat_avg']:.1f} vs 高弹性热度 {high_beta['social_heat_avg']:.1f}")

            monitor.update({
                "status": result.status,
                "phase": result.stage_label,
                "structure": result.structure_label,
                "strategy": result.strategy_advice,
                "risk_lines": result.risk_triggers[:4],
                "signals": signals[:4],
                "bubble_heat": result.bubble_heat,
                "break_risk": result.break_risk,
            })
            return monitor
        except Exception as e:
            logger.warning(f"[大盘] 构建 AI 拥挤度监控失败: {e}")
            return monitor

    def _build_ai_bubble_block(self, overview: MarketOverview) -> str:
        monitor = getattr(overview, 'ai_bubble_monitor', None) or {}
        if self.region != 'us' or not monitor:
            return ''
        lines = [
            '### 六、AI拥挤度监控',
            '',
            f"- 当前状态：{monitor.get('status', '数据不足')}（热度 {monitor.get('bubble_heat', 'N/A')}/100｜风险 {monitor.get('break_risk', 'N/A')}/100）",
            f"- 阶段判断：{monitor.get('phase', '等待更多样本')}",
            f"- 结构特征：{monitor.get('structure', '结构信号尚未统一')}",
            f"- 策略建议：{monitor.get('strategy', '维持现有主线跟踪，等待更多确认信号。')}",
        ]
        signals = monitor.get('signals', []) or []
        if signals:
            lines.append('- 监控信号：' + '；'.join(signals[:3]))
        risk_lines = monitor.get('risk_lines', []) or []
        if risk_lines:
            lines.append('- 风控线：' + ' / '.join(risk_lines[:4]))
        lines.append('')
        return '\n'.join(lines)
    ...
    def _build_news_block(self, news: List) -> str:
        """Temporarily disable low-quality catalyst block without affecting other sections."""
        return ""
