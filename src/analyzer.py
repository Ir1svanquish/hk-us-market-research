...
def _is_meaningful_analysis_summary(value: Any) -> bool:
    """Return False for parser/integrity placeholders that contain no conclusion."""
    if not isinstance(value, str):
        return False
    normalized = value.strip().lower().strip("。.!！?？；;，, ")
    return normalized not in _INVALID_ANALYSIS_SUMMARIES
...
def check_content_integrity(result: "AnalysisResult") -> Tuple[bool, List[str]]:
    ...
    if not _is_meaningful_analysis_summary(summary):
...
"""
    ...
    def _get_analysis_system_prompt(self, report_language: str, stock_code: str = "") -> str:
        ...
        currency_unit = get_market_currency_unit(stock_code, lang)
        ...
        base_prompt = base_prompt.replace("XX元", f"XX{currency_unit}")
        if lang == "en":
            return base_prompt + """
...
"""
    ...
    def _dispatch_litellm_completion(
        ...
        *,
        ...
        effective_kwargs.update(extra_params)
        if extra_params.get("custom_llm_provider") == "openai" and model.startswith("deepseek/"):
            effective_kwargs["model"] = model.split("/", 1)[1]
    ...
    def analyze(
        ...
        try:
            ...
            while True:
                ...
                self._normalize_result_trade_currency(result, code, report_language)
                result.market_snapshot = self._build_market_snapshot(context)
                self._apply_market_snapshot_guard(result, report_language)
    ...
    def _format_prompt(
        ...
        currency_unit = get_market_currency_unit(code, report_language)
...
"""
        ...
        finnhub_earnings = earnings_data.get("finnhub_earnings", {}) if isinstance(earnings_data, dict) else {}
        finnhub_earnings_summary = finnhub_earnings.get("earnings_summary", {}) if isinstance(finnhub_earnings, dict) else {}
        finnhub_next_earnings = finnhub_earnings.get("next_earnings", {}) if isinstance(finnhub_earnings, dict) else {}
        hk_profile = earnings_data.get("hk_profile", {}) if isinstance(earnings_data, dict) else {}
        hk_history_metrics = earnings_data.get("hk_history_metrics", {}) if isinstance(earnings_data, dict) else {}
        institution_block = (
        ...
        finnhub_recommendation = institution_data.get("finnhub_recommendation", {}) if isinstance(institution_data, dict) else {}
        hk_connect_holding = institution_data.get("hk_connect_holding", {}) if isinstance(institution_data, dict) else {}
        if isinstance(finnhub_earnings_summary, dict) and any(v not in (None, '', 'N/A') for v in finnhub_earnings_summary.values()):
            prompt += f"""
### 美股财报跟踪（Finnhub）
| 指标 | 数值 | 说明 |
|------|------|------|
| 最近财报期 | {finnhub_earnings_summary.get('latest_period', 'N/A')} | 最近一个已披露季度 |
| 最新实际 EPS | {finnhub_earnings_summary.get('latest_actual', 'N/A')} | |
| 市场预期 EPS | {finnhub_earnings_summary.get('latest_estimate', 'N/A')} | |
| 近4季正向 surprise | {finnhub_earnings_summary.get('positive_surprise_quarters', 'N/A')} | 次数 |
| 近4季负向 surprise | {finnhub_earnings_summary.get('negative_surprise_quarters', 'N/A')} | 次数 |
"""
        if isinstance(finnhub_next_earnings, dict) and any(v not in (None, '', 'N/A') for v in finnhub_next_earnings.values()):
            prompt += f"""
### 美股下一次财报窗口（Finnhub）
| 指标 | 数值 | 说明 |
|------|------|------|
| 财报日期 | {finnhub_next_earnings.get('date', 'N/A')} | |
| 预估 EPS | {finnhub_next_earnings.get('eps_estimate', 'N/A')} | 免费权限可拿到的日历值 |
| 披露时段 | {finnhub_next_earnings.get('hour', 'N/A')} | bmo / amc / dmh |
| 财报季度 | {finnhub_next_earnings.get('year', 'N/A')}Q{finnhub_next_earnings.get('quarter', 'N/A')} | |
"""
        if isinstance(finnhub_recommendation, dict) and any(v not in (None, '', 'N/A') for v in finnhub_recommendation.values()):
            delta = finnhub_recommendation.get('change_vs_prev', {}) if isinstance(finnhub_recommendation.get('change_vs_prev'), dict) else {}
            prompt += f"""
### 美股机构推荐趋势（Finnhub）
| 指标 | 数值 | 说明 |
|------|------|------|
| 统计期 | {finnhub_recommendation.get('period', 'N/A')} | 月度汇总 |
| Strong Buy / Buy | {finnhub_recommendation.get('strong_buy', 'N/A')} / {finnhub_recommendation.get('buy', 'N/A')} | |
| Hold / Sell / Strong Sell | {finnhub_recommendation.get('hold', 'N/A')} / {finnhub_recommendation.get('sell', 'N/A')} / {finnhub_recommendation.get('strong_sell', 'N/A')} | |
| 与上期变化 | SB {delta.get('strong_buy', 'N/A')}, B {delta.get('buy', 'N/A')}, H {delta.get('hold', 'N/A')}, S {delta.get('sell', 'N/A')} | |
"""
        if isinstance(hk_profile, dict) and hk_profile:
            prompt += f"""
### 港股基础资料（Tushare）
| 指标 | 数值 | 说明 |
|------|------|------|
| 股票全称 | {hk_profile.get('name', 'N/A')} | 港股基础资料 |
| 上市日期 | {hk_profile.get('list_date', 'N/A')} | YYYYMMDD |
| 交易单位 | {hk_profile.get('trade_unit', 'N/A')} | 每手股数 |
| ISIN | {hk_profile.get('isin', 'N/A')} | 国际证券识别码 |
| 币种 | {hk_profile.get('currency', 'N/A')} | |
"""
        if isinstance(hk_history_metrics, dict) and hk_history_metrics:
            prompt += f"""
### 港股历史衍生字段（Tushare）
| 指标 | 数值 | 说明 |
|------|------|------|
| 最新收盘价 | {hk_history_metrics.get('latest_close', 'N/A')} | 历史日线最新收盘 |
| 单日涨跌幅 | {hk_history_metrics.get('day_change_pct', 'N/A')} | 相对上一交易日 |
| 60日涨跌幅 | {hk_history_metrics.get('change_60d', 'N/A')} | 中期表现 |
| 60日区间高点 | {hk_history_metrics.get('range_60d_high', 'N/A')} | |
| 60日区间低点 | {hk_history_metrics.get('range_60d_low', 'N/A')} | |
| 20日平均成交额 | {hk_history_metrics.get('avg_amount_20d', 'N/A')} | |
"""
        if isinstance(hk_connect_holding, dict) and hk_connect_holding:
            prompt += f"""
### 港股通持股（Tushare）
| 指标 | 数值 | 说明 |
|------|------|------|
| 最新披露日 | {hk_connect_holding.get('trade_date', 'N/A')} | |
| 持股数量 | {hk_connect_holding.get('hold_vol', 'N/A')} | |
| 持股比例 | {hk_connect_holding.get('hold_ratio', 'N/A')} | % |
| 交易所 | {hk_connect_holding.get('exchange', 'N/A')} | |
"""
...
"""
    ...
    def _format_amount(
        ...
        """Format turnover/market value in the listing currency."""
        if amount is None:
            return 'N/A'
        try:
        ...
        if language == "en":
            ...
            if abs(numeric) >= 1e9:
                return f"{currency} {numeric / 1e9:.2f}B"
            if abs(numeric) >= 1e6:
                return f"{currency} {numeric / 1e6:.2f}M"
            if abs(numeric) >= 1e3:
                return f"{currency} {numeric / 1e3:.2f}K"
        ...
        if abs(numeric) >= 1e8:
            return f"{numeric / 1e8:.2f} {units[2]}"
        if abs(numeric) >= 1e4:
            return f"{numeric / 1e4:.2f} {units[1]}"
        return f"{numeric:.0f} {units[0]}"
    ...
    def _format_price(
        ...
        """Format a quote price, optionally including its listing currency."""
        if value is None:
            return 'N/A'
        try:
        ...
        if not stock_code:
            return number
        unit = get_market_currency_unit(stock_code, report_language)
        if normalize_report_language(report_language) == "en":
            return f"{unit} {number}"
        return f"{number} {unit}"

    def _build_market_snapshot(self, context: Dict[str, Any]) -> Dict[str, Any]:
        """Build a same-source market snapshot and recompute derived fields."""
        today = context.get('today', {}) or {}
        realtime = context.get('realtime', {}) or {}
        yesterday = context.get('yesterday', {}) or {}
        stock_code = str(context.get('code') or '')
        ...
        # When realtime data exists, keep OHLC/prev-close/amount on that same
        # quote snapshot. Historical bars are fallback only.
        close = _number(realtime.get('price'), positive=True) or _number(today.get('close'), positive=True)
        prev_close = _number(realtime.get('prev_close'), positive=True) or _number(
            yesterday.get('close'), positive=True
        )
        open_price = _number(realtime.get('open'), positive=True) or _number(today.get('open'), positive=True)
        high = _number(realtime.get('high'), positive=True) or _number(today.get('high'), positive=True)
        low = _number(realtime.get('low'), positive=True) or _number(today.get('low'), positive=True)
        volume = realtime.get('volume') if realtime.get('volume') is not None else today.get('volume')
        amount = realtime.get('amount') if realtime.get('amount') is not None else today.get('amount')
        ...
        if prev_close is not None and close is not None:
            ...
            pct_chg = change_amount / prev_close * 100
        if prev_close is not None and high is not None and low is not None:
            amplitude = (high - low) / prev_close * 100

        raw_pct = _number(realtime.get('change_pct'))
        if raw_pct is None:
            raw_pct = _number(today.get('pct_chg'))
        raw_change = _number(realtime.get('change_amount'))
        raw_amplitude = _number(realtime.get('amplitude'))

        conflicts: List[str] = []
        if raw_pct is not None and pct_chg is not None and abs(raw_pct - pct_chg) > 0.15:
            conflicts.append(f"涨跌幅原值{raw_pct:.2f}%与重算值{pct_chg:.2f}%不一致")
        if raw_change is not None and change_amount is not None:
            tolerance = max(0.02, abs(prev_close or 0) * 0.001)
            if abs(raw_change - change_amount) > tolerance:
                conflicts.append("涨跌额与收盘/昨收不一致")
        if raw_amplitude is not None and amplitude is not None and abs(raw_amplitude - amplitude) > 0.15:
            conflicts.append("振幅与最高/最低/昨收不一致")

        consistency_warning = None
        if conflicts:
            consistency_warning = (
                "Market quote fields conflict; trading decision downgraded pending verification."
                if report_language == "en"
                else "行情字段存在冲突，交易结论已降级，等待数据核验。"
            )

        snapshot = {
            "date": context.get('date', '未知'),
            "close": self._format_price(close, stock_code, report_language),
            "open": self._format_price(open_price, stock_code, report_language),
            "high": self._format_price(high, stock_code, report_language),
            "low": self._format_price(low, stock_code, report_language),
            "prev_close": self._format_price(prev_close, stock_code, report_language),
            "pct_chg": self._format_percent(pct_chg if pct_chg is not None else raw_pct),
            "change_amount": self._format_price(change_amount, stock_code, report_language),
            "amplitude": self._format_percent(amplitude),
            "volume": self._format_volume(volume),
            "amount": self._format_amount(amount, market, report_language),
            "currency_unit": get_market_currency_unit(stock_code, report_language),
            "data_consistent": not conflicts,
            "consistency_details": conflicts,
            "consistency_warning": consistency_warning,
        }

        if realtime:
            snapshot.update({
                "price": self._format_price(close, stock_code, report_language),
    ...
    @staticmethod
    def _normalize_currency_text(value: Any, currency_unit: str) -> Any:
        """Normalize bare numeric ``元`` references in trading guidance."""
        if not isinstance(value, str) or not value or currency_unit == "元":
            return value
        return re.sub(r"(?<=\d)元", currency_unit, value)

    def _normalize_result_trade_currency(
        self,
        result: AnalysisResult,
        stock_code: str,
        report_language: str,
    ) -> None:
        """Keep actionable prices in the listing currency even if the LLM says 元."""
        ...
        for field in ("analysis_summary", "key_points", "risk_warning", "buy_reason"):
            setattr(
                result,
                field,
                self._normalize_currency_text(getattr(result, field, None), currency_unit),
            )
        ...
        def _walk(value: Any) -> Any:
            ...
            return self._normalize_currency_text(value, currency_unit)

        # Normalize decision/action text only. Intelligence may legitimately
        # quote financial figures reported in a different currency.
        for section in ("core_conclusion", "battle_plan"):
            if section in dashboard:
                dashboard[section] = _walk(dashboard[section])
        result.dashboard = dashboard

    @staticmethod
    def _apply_market_snapshot_guard(result: AnalysisResult, report_language: str) -> None:
        """Suppress actionable advice when quote fields still contradict each other."""
        snapshot = result.market_snapshot if isinstance(result.market_snapshot, dict) else {}
        if snapshot.get("data_consistent", True):
            return

        is_english = normalize_report_language(report_language) == "en"
        warning = snapshot.get("consistency_warning") or (
            "Market quote data conflict; pending verification."
            if is_english
            else "行情数据冲突，等待核验。"
        )
        result.operation_advice = "Watch" if is_english else "观望"
        result.decision_type = "hold"
        result.confidence_level = "Low" if is_english else "低"
        try:
            result.sentiment_score = min(int(result.sentiment_score), 50)
        except (TypeError, ValueError):
            result.sentiment_score = 50
        ...
        core["signal_type"] = "⚪ Watch" if is_english else "⚪观望"
        core["time_sensitivity"] = "Pending data verification" if is_english else "等待数据核验"
        core["position_advice"] = {
            "no_position": "Do not open a position until quote data is verified."
            if is_english
            else "行情核验完成前不开仓。",
            "has_position": "Keep existing risk controls; do not add based on this report."
            if is_english
            else "仅执行原有风控，不依据本报告加仓。",
        }
        dashboard["core_conclusion"] = core

        intelligence = dashboard.get("intelligence") if isinstance(dashboard.get("intelligence"), dict) else {}
        risks = intelligence.get("risk_alerts") if isinstance(intelligence.get("risk_alerts"), list) else []
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                return AnalysisResult(
                    ...
                    # 综合
                    # Keep a missing summary empty so the integrity gate can retry it.
                    # A success-looking fallback such as "分析完成" hides incomplete LLM output.
