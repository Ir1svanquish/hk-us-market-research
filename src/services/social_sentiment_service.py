...
class SocialSentimentService:
    ...
    def _headers_for_key(self, api_key: Optional[str]) -> Dict[str, str]:
    ...
    def _fetch_json(self, url: str, params: Optional[Dict[str, Any]] = None) -> Optional[Dict]:
        """Fetch JSON from API, rotating across configured keys."""
        for idx, api_key in enumerate(self._api_keys or [None], 1):
            try:
                resp = _get_with_retry(url, headers=self._headers_for_key(api_key), params=params)
                if resp.status_code == 200:
                    record_api_result('SocialSentiment', api_key, success=True)
                    return resp.json()
                logger.warning("Social sentiment API %s key#%s returned %s", url, idx, resp.status_code)
                record_api_result('SocialSentiment', api_key, success=False, error=f'HTTP {resp.status_code}')
                if resp.status_code in (401, 403):
                    continue
                return None
            except _TRANSIENT_EXCEPTIONS as e:
                record_api_result('SocialSentiment', api_key, success=False, error=str(e))
                logger.warning("Social sentiment API %s network error with key#%s: %s", url, idx, e)
            except Exception as e:
                record_api_result('SocialSentiment', api_key, success=False, error=str(e))
                logger.warning("Social sentiment API %s unexpected error with key#%s: %s", url, idx, e)
        return None
    ...
    def fetch_reddit_report(self, ticker: str) -> Optional[Dict]:
        """Derive a per-ticker Reddit entry from trending data."""
        trending = self.fetch_reddit_trending()
        if not trending:
            return None
        return self._find_ticker_in_trending(trending, ticker.upper())

    def fetch_reddit_trending(self) -> Optional[List[Dict]]:
        """Fetch Reddit trending stocks (cached)."""
        url = f"{self._api_url}/reddit/stocks/v1/trending?days=7&limit=100"
    ...
    def get_social_data(self, ticker: str) -> Optional[Dict[str, Optional[Dict]]]:
    ...
    def get_social_context(self, ticker: str) -> Optional[str]:
        ...
        """
        social_data = self.get_social_data(ticker)
        if not social_data:
            return None
        return self._format_social_intel(
            social_data['ticker'],
            social_data.get('reddit'),
            social_data.get('x'),
            social_data.get('polymarket'),
        )
    ...
    @staticmethod
    def build_summary_lines(social_data: Optional[Dict[str, Optional[Dict]]]) -> List[str]:
        if not social_data:
            return []

        lines: List[str] = []
        reddit = social_data.get('reddit')
        x_entry = social_data.get('x')
        poly = social_data.get('polymarket')

        if reddit:
            report = reddit.get('report', reddit)
            buzz = SocialSentimentService._coalesce(report.get('buzz_score'), report.get('buzz'))
            sentiment = SocialSentimentService._coalesce(report.get('sentiment_score'), report.get('sentiment'))
            mentions = SocialSentimentService._coalesce(report.get('total_mentions'), report.get('mentions'))
            parts = []
            if buzz is not None:
                parts.append(f'热度 {buzz}/100')
            if sentiment is not None:
                parts.append(f'情绪 {sentiment}')
            if mentions is not None:
                parts.append(f'提及 {mentions}')
            if parts:
                lines.append('Reddit：' + '，'.join(parts))

        if x_entry:
            buzz = SocialSentimentService._coalesce(x_entry.get('buzz_score'), x_entry.get('buzz'))
            sentiment = SocialSentimentService._coalesce(x_entry.get('sentiment_score'), x_entry.get('sentiment'))
            mentions = SocialSentimentService._coalesce(x_entry.get('total_mentions'), x_entry.get('mentions'))
            parts = []
            if buzz is not None:
                parts.append(f'热度 {buzz}/100')
            if sentiment is not None:
        ...
        if poly:
            buzz = SocialSentimentService._coalesce(poly.get('buzz_score'), poly.get('buzz'))
            sentiment = SocialSentimentService._coalesce(poly.get('sentiment_score'), poly.get('sentiment'))
            trades = SocialSentimentService._coalesce(poly.get('trade_count'), poly.get('trades'))
            parts = []
            if buzz is not None:
                parts.append(f'热度 {buzz}/100')
            if sentiment is not None:
                parts.append(f'预期 {sentiment}')
            if trades is not None:
                parts.append(f'交易数 {trades}')
            if parts:
                lines.append('Polymarket：' + '，'.join(parts))
