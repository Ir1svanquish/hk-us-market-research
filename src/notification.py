...
class NotificationService(
    ...
    def generate_daily_report(
        ...
        if report_date is None:
            report_date = now_utc8.strftime('%Y-%m-%d')
        ...
        report_lines = [
            ...
            f"{labels['generated_at_label']}：{now_utc8.strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
        ...
        report_lines.extend([
            "",
            f"*{labels['generated_at_label']}：{_now_utc8().strftime('%Y-%m-%d %H:%M:%S')}*",
        ])
    ...
    def generate_dashboard_report(
        ...
        # 统计信息 - 与下方逐条图标完全一致
        signal_counts = {"🟢": 0, "🟡": 0, "⚪": 0, "🟠": 0, "🔴": 0}
        for r in results:
            _, emoji, _ = self._get_signal_level(r)
            signal_counts[emoji] = signal_counts.get(emoji, 0) + 1

        report_lines = [
            f"# 🎯 {report_date} {labels['dashboard_title']}",
            "",
            f"> {labels['analyzed_prefix']} **{len(results)}** {labels['stock_unit']} | "
            f"🟢买入:{signal_counts['🟢']} 🟡持有:{signal_counts['🟡']} ⚪观望:{signal_counts['⚪']} 🟠减仓:{signal_counts['🟠']} 🔴卖出:{signal_counts['🔴']}",
            "",
        ]
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                if intel:
                    ...
                    if earnings_outlook:
                        raw_earnings_outlook = str(earnings_outlook).strip()
                        low = raw_earnings_outlook.lower()
                        hk_no_data_markers = (
                            '数据缺失，无法判断', '数据缺失,无法判断', '暂无数据，无法判断',
                            '财报数据缺失，无法评估', '财报数据缺失,无法评估',
                            '数据缺失，无法分析业绩预期', '财务数据缺失，无法分析业绩预期', '财报数据缺失，无法分析业绩预期',
                            '无法评估', 'n/a', 'none'
                        )
                        hk_placeholder_markers = (
                            '未找到近期业绩预告或快报',
                            '未找到业绩预告或快报',
                            '未找到近期业绩预告',
                        )
                        is_hk = str(result.code).lower().startswith('hk')
                        if is_hk and (
                            any(marker in low for marker in hk_no_data_markers)
                            or any(marker in raw_earnings_outlook for marker in hk_placeholder_markers)
                        ):
                            realtime_pb = getattr(result, 'pb_ratio', None)
                            realtime_pe = getattr(result, 'pe_ratio', None)
                            if isinstance(realtime_pe, (int, float)) and realtime_pe > 0:
                                if realtime_pe >= 80:
                                    earnings_outlook = f'估值显著偏高，后续更看业绩增长兑现；PE约{realtime_pe}。'
                                elif realtime_pe >= 30:
                                    earnings_outlook = f'盈利基础尚可，后续关注增长持续性与估值消化；PE约{realtime_pe}。'
                                else:
                                    earnings_outlook = f'当前估值处于相对可跟踪区间，后续重点看盈利延续性；PE约{realtime_pe}。'
                            elif isinstance(realtime_pb, (int, float)) and realtime_pb > 0:
                                if realtime_pb >= 5:
                                    earnings_outlook = f'账面支撑有限且估值不低，后续更看经营兑现；PB约{realtime_pb}。'
                                else:
                                    earnings_outlook = f'账面支撑仍在，后续关注经营改善节奏；PB约{realtime_pb}。'
                            else:
                                earnings_outlook = '后续重点看经营兑现与价格趋势共振。'
                        report_lines.append(f"**📊 {labels['earnings_outlook_label']}**: {earnings_outlook}")
                    social_summary = intel.get('social_sentiment_summary', [])
                    if social_summary:
                        report_lines.append("")
                        report_lines.append("**📱 社交情绪摘要**:")
                        for line in social_summary[:4]:
                ...
                if isinstance(data_persp, dict) and data_persp:
                    ...
                    if price_data:
                        ...
                        currency_unit = (
                            result.market_snapshot.get('currency_unit')
                            if isinstance(getattr(result, 'market_snapshot', None), dict)
                            else ''
                        )
                        if currency_unit:
                            price_value_label = (
                                f"{labels['current_price_label']}（{currency_unit}）"
                                if report_language == 'zh'
                                else f"{labels['current_price_label']} ({currency_unit})"
                            )
                        else:
                            price_value_label = labels['current_price_label']
                        report_lines.extend([
                            f"| {labels['price_metrics_label']} | {price_value_label} |",
                            "|---------|------|",
                            f"| {price_value_label} | {price_data.get('current_price', 'N/A')} |",
                ...
                if battle:
                    ...
                    if sniper:
                        ...
                        report_lines.extend([
                            ...
                            "",
                            f"| {labels['action_points_heading']} | {price_value_label} |",
                            "|---------|------|",
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                checklist = battle.get('action_checklist', []) if battle else []
                if checklist:
                    checklist = [c for c in checklist if '筹码健康' not in str(c)]
                    # 只显示不通过的项目
    ...
    def generate_brief_report(
        ...
        lines = [
            ...
            "",
            f"> {len(results)} {labels['stock_unit_compact']} | 🟢{signal_counts['🟢']} 🟡{signal_counts['🟡']} ⚪{signal_counts['⚪']} 🟠{signal_counts['🟠']} 🔴{signal_counts['🔴']}",
            "",
        ]
        for r in sorted_results:
            _, emoji, _ = self._get_signal_level(r)
        ...
        lines.append("")
        lines.append(f"*{_now_utc8().strftime('%Y-%m-%d %H:%M:%S')}*")
        return "\n".join(lines)
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        ...
        Returns:
            Markdown 格式的单股报告
        """
        report_date = _now_utc8().strftime('%Y-%m-%d %H:%M')
    ...
    def _append_market_snapshot(self, lines: List[str], result: AnalysisResult) -> None:
        ...
        if snapshot.get('consistency_warning'):
            lines.extend(["", f"> ⚠️ {snapshot.get('consistency_warning')}"])
    ...
    def send(
        ...
        skip_channel_set = {str(ch).lower() for ch in (skip_channels or [])}
        target_channels = [ch for ch in self._available_channels if ch.value.lower() not in skip_channel_set]
        channel_names = [ChannelDetector.get_channel_name(ch) for ch in target_channels]
        ...
        for channel in target_channels:
            ...
            try:
                if channel == NotificationChannel.WECHAT:
                    ...
                    if self.is_detail_content_blocked_for_telegram(content):
                        logger.info("Telegram 正文已拦截 detailed content，跳过旧文本发送")
                        result = False
                    elif use_image:
    ...
    def save_report_to_file(
        ...
        market_suffix = (getattr(get_config(), 'market_review_region', 'cn') or 'cn').lower()
        if filename is None:
            date_str = datetime.now().strftime('%Y%m%d')
            filename = f"report_{date_str}_{market_suffix}.md"
        elif filename.startswith(('report_', 'market_review_', 'compact_report_')) and filename.endswith('.md'):
            stem = filename[:-3]
            if not stem.endswith(f'_{market_suffix}'):
                filename = f"{stem}_{market_suffix}.md"
