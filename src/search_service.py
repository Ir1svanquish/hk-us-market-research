...
from src.api_key_cooldown import (
    cooldown as mark_key_cooldown,
    quarantine as quarantine_api_key,
    is_cooled_down,
    increment_probe_failure,
    reset_probe_failure,
    PROBE_FAILURE_THRESHOLD,
)
from src.api_usage_tracker import record_api_result
...
_TAVILY_AUTH_ERROR_TOKENS = (
    ...
    'deactivated',
    'disabled',
    'account associated with this api key',
)

_TAVILY_QUOTA_ERROR_TOKENS = (
    'rate limit',
    'quota',
    'usage limit',
    'insufficient_quota',
    'used up all your searches',
    ...
    def _get_next_key(self) -> Optional[str]:
        ...
        with self._state_lock:
            ...
            for _ in range(len(self._api_keys)):
                ...
                if is_cooled_down(self._name, key):
                    continue
                # 跳过错误次数过多的 key（超过 3 次）
                if self._key_errors.get(key, 0) < 3:
                    return key
            
            # 所有 key 都有问题，重置错误计数并尝试返回第一个未冷却 key
            logger.warning(f"[{self._name}] 所有 API Key 都有错误记录或处于冷却中，重置错误计数")
            self._key_errors = {key: 0 for key in self._api_keys}
            for key in self._api_keys:
                if not is_cooled_down(self._name, key):
                    return key
            return None
        ...
        **search_kwargs: Any,
        ...
        try:
            ...
            if response.success:
                ...
                record_api_result(self._name, api_key, success=True)
                reset_probe_failure(self._name, api_key)
                logger.info(f"[{self._name}] 搜索 '{query}' 成功，返回 {len(response.results)} 条结果，耗时 {response.search_time:.2f}s")
            else:
                self._record_error(api_key)
                record_api_result(self._name, api_key, success=False, error=response.error_message)
            ...
            record_api_result(self._name, api_key, success=False, error=str(e))
...
class TavilySearchProvider(BaseSearchProvider):
    ...
    def _do_search(
        ...
        try:
            ...
            if any(token in lowered for token in _TAVILY_AUTH_ERROR_TOKENS):
                quarantine_api_key(self.name, api_key, reason=error_msg)
                logger.warning(f"[{self.name}] API Key {api_key[:8]}... 认证失效，已持久隔离")
                error_msg = f"API Key 认证失效，已持久隔离: {error_msg}"
            elif any(token in lowered for token in _TAVILY_QUOTA_ERROR_TOKENS):
                mark_key_cooldown(self.name, api_key, days=30)
                logger.warning(f"[{self.name}] API Key {api_key[:8]}... 已放入30天冷却池")
...
class SerpAPISearchProvider(BaseSearchProvider):
    ...
    def _do_search(self, query: str, api_key: str, max_results: int, days: int = 7) -> SearchResponse:
        ...
        try:
            ...
            if not results and self._looks_suspicious_zero_results(query=query, response=response):
                probe_error = self._probe_key_health(api_key)
                if probe_error:
                    fail_count = increment_probe_failure(self.name, api_key)
                    if fail_count >= PROBE_FAILURE_THRESHOLD:
                        mark_key_cooldown(self.name, api_key, days=1)
                        logger.warning(f"[{self.name}] API Key {api_key[:8]}... 零结果且探针失败 {fail_count}/{PROBE_FAILURE_THRESHOLD} 次，已放入1天冷却池")
                        return SearchResponse(
                            query=query,
                            results=[],
                            provider=self.name,
                            success=False,
                            error_message=f"API Key 可疑失效/额度用尽: {probe_error}"
                        )
                    else:
                        logger.warning(f"[{self.name}] API Key {api_key[:8]}... 探针失败 {fail_count}/{PROBE_FAILURE_THRESHOLD}，暂不冷却")
            ...
            if any(token in lowered for token in (
                'rate limit', 'quota', 'usage limit', 'insufficient_quota', 'plan',
                'deactivated', 'subscription', 'disabled', 'account associated with this api key',
                'used up all your searches', 'searches for the month', 'monthly searches',
                'monthly limit', 'upgrade your plan', 'scheduled to renew'
            )):
                mark_key_cooldown(self.name, api_key, days=30)
                logger.warning(f"[{self.name}] API Key {api_key[:8]}... 已放入30天冷却池")
                error_msg = f"API 配额已用尽: {error_msg}"
            elif self._looks_suspicious_zero_results(query=query, response=None):
                probe_error = self._probe_key_health(api_key)
                if probe_error:
                    fail_count = increment_probe_failure(self.name, api_key)
                    if fail_count >= PROBE_FAILURE_THRESHOLD:
                        mark_key_cooldown(self.name, api_key, days=1)
                        logger.warning(f"[{self.name}] API Key {api_key[:8]}... 探针失败 {fail_count}/{PROBE_FAILURE_THRESHOLD} 次，已放入1天冷却池")
                    else:
                        logger.warning(f"[{self.name}] API Key {api_key[:8]}... 探针失败 {fail_count}/{PROBE_FAILURE_THRESHOLD}，暂不冷却")
                    error_msg = f"API Key 可疑失效/额度用尽: {probe_error}"
            return SearchResponse(
                query=query,
                results=[],
                provider=self.name,
    ...
    @staticmethod
    def _looks_suspicious_zero_results(query: str, response: Optional[Dict[str, Any]]) -> bool:
        q = (query or '').lower()
        if any(token in q for token in ('analyst', 'rating', 'target price', 'earnings', 'revenue', 'forecast')):
            return True
        if response:
            info = str(response).lower()
            if any(token in info for token in ('used up all your searches', 'monthly searches', 'upgrade your plan', 'scheduled to renew')):
                return True
        return False

    def _probe_key_health(self, api_key: str) -> Optional[str]:
        try:
            from serpapi import GoogleSearch
            params = {
                'engine': 'google',
            ...
            knowledge_graph = response.get('knowledge_graph') or {}
            if organic or answer_box or knowledge_graph:
                return None
            return 'probe returned no useful results'
        except Exception as e:
            return str(e)
...
class SearchService:
            ...
            for provider in self._providers:
                ...
                if filtered_response.success and filtered_response.results:
                    ...
                    if response.success and not filtered_response.results:
                        zero_result_providers += 1
                        logger.info(
                            "%s 搜索成功但过滤后无有效新闻，继续尝试下一引擎",
                            provider.name,
                        )
                        if zero_result_providers >= 2:
                            logger.info("连续两个搜索引擎都未提供有效新闻，停止继续尝试以减少无效调用")
                            break
                    else:
                        logger.warning(
                            "%s 搜索失败: %s，尝试下一个引擎",
    ...
    def search_comprehensive_intel(
        ...
        # 轮流使用不同的搜索引擎
        provider_index = 0
        low_value_dims = {'market_analysis', 'earnings', 'industry'}
        ...
        for dim in search_dimensions:
            ...
            provider_index += 1
            if dim['name'] in low_value_dims and provider.name == 'Tavily' and len(available_providers) > 1:
                provider = next((p for p in available_providers if p.name != 'Tavily'), provider)
