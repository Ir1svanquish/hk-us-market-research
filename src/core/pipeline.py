...
class StockAnalysisPipeline:
    ...
    def analyze_stock(self, code: str, report_type: ReportType, query_id: str) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            # 名称规范化：优先使用 PREFERRED_NAME_OVERRIDE 中的显示名称
            from src.data.stock_mapping import normalize_stock_name
            stock_name = normalize_stock_name(code, stock_name)
            ...
            if getattr(self.config, 'enable_chip_distribution', True):
                ...
                logger.info(f"{stock_name}({code}) 筹码分布已通过配置关闭，跳过")
            ...
            if self.social_sentiment_service is not None and self.social_sentiment_service.is_available and is_us_stock_code(code):
                try:
                    social_data = self.social_sentiment_service.get_social_data(code)
            ...
            if is_hk_stock_code(code) and realtime_quote is not None:
                fc = enhanced_context.get('fundamental_context') or {}
                earnings_block = fc.get('earnings') or {}
                earnings_data = earnings_block.get('data') or {}
                financial_report = earnings_data.get('financial_report')
                has_financial = isinstance(financial_report, dict) and any(
                    financial_report.get(k) not in (None, '', 'N/A')
                    for k in ('revenue', 'net_profit_parent', 'operating_cash_flow', 'roe')
                )
                if not has_financial:
                    lb_proxy = {
                        'eps_ttm': getattr(realtime_quote, 'pe_ratio', None),
                        'pb_ratio': getattr(realtime_quote, 'pb_ratio', None),
                        'price': getattr(realtime_quote, 'price', None),
                    }
                    earnings_data['longbridge_proxy'] = lb_proxy
                    earnings_block['data'] = earnings_data
                    fc['earnings'] = earnings_block
                    enhanced_context['fundamental_context'] = fc
            ...
            if result:
                ...
                result.pe_ratio = realtime_data.get('pe_ratio')
                result.pb_ratio = realtime_data.get('pb_ratio')

            # Step 7.6: chip_structure fallback (Issue #589)
            if result and chip_data:
                fill_chip_structure_if_needed(result, chip_data)

            # Step 7.65: attach social sentiment summary for rendering
            if result:
                if not getattr(result, 'dashboard', None):
                    result.dashboard = {}
                intel = result.dashboard.get('intelligence') or {}
                if social_data:
                    intel['social_sentiment_summary'] = self.social_sentiment_service.build_summary_lines(social_data)

                fc = enhanced_context.get('fundamental_context') or {}
                earnings_block = fc.get('earnings') or {}
                institution_block = fc.get('institution') or {}
                earnings_data = earnings_block.get('data') or {}
                institution_data = institution_block.get('data') or {}
                finnhub_earnings = earnings_data.get('finnhub_earnings') or {}
                finnhub_summary = finnhub_earnings.get('earnings_summary') or {}
                finnhub_next = finnhub_earnings.get('next_earnings') or {}
                finnhub_rec = institution_data.get('finnhub_recommendation') or {}
                if finnhub_summary and str(result.code).upper() and not str(result.code).upper().startswith('HK'):
                    parts = []
                    latest_period = finnhub_summary.get('latest_period')
                    pos_q = finnhub_summary.get('positive_surprise_quarters')
                    neg_q = finnhub_summary.get('negative_surprise_quarters')
                    latest_actual = finnhub_summary.get('latest_actual')
                    latest_estimate = finnhub_summary.get('latest_estimate')
                    if finnhub_next.get('date'):
                        next_hour = finnhub_next.get('hour')
                        if next_hour not in (None, '', 'N/A'):
                            parts.append(f"下一次财报日 {finnhub_next.get('date')}（{next_hour}）")
                        else:
                            parts.append(f"下一次财报日 {finnhub_next.get('date')}")
                    if latest_period:
                        # Finnhub's `period` is a fiscal period end, not a
                        # calendar-quarter label.  Do not derive Q1/Q2/Q3/Q4
                        # from its month because non-calendar fiscal years
                        # would be mislabeled.
                        parts.append(f"最近已披露业绩期（期末 {latest_period}）")
                    if latest_actual not in (None, '', 'N/A') and latest_estimate not in (None, '', 'N/A'):
                        parts.append(f"实际EPS {latest_actual}，预期 {latest_estimate}")
                    if pos_q not in (None, '', 'N/A') and neg_q not in (None, '', 'N/A'):
                        parts.append(f"近4季超预期 {pos_q} 次、低于预期 {neg_q} 次")
                    if finnhub_rec.get('period'):
                        sb = finnhub_rec.get('strong_buy', 'N/A')
                        buy = finnhub_rec.get('buy', 'N/A')
                        hold = finnhub_rec.get('hold', 'N/A')
                        sell = finnhub_rec.get('sell', 'N/A')
                        parts.append(f"机构推荐（{finnhub_rec.get('period')}）：强买 {sb} / 买入 {buy} / 持有 {hold} / 卖出 {sell}")
                    if parts:
    ...
    def _enhance_context(
        ...
        if realtime_quote:
            ...
            enhanced['realtime'] = {
                ...
                'prev_close': getattr(realtime_quote, 'pre_close', None),
        ...
        if realtime_quote and trend_result and trend_result.ma5 > 0:
            ...
            if price is not None and price > 0:
                ...
                realtime_prev_close = getattr(realtime_quote, 'pre_close', None)
                if realtime_prev_close not in (None, 0):
                    try:
                        realtime_prev_close = float(realtime_prev_close)
                        pct = round(
                            (float(price) - realtime_prev_close) / realtime_prev_close * 100,
                            2,
                        )
                ...
                price_change_base = realtime_prev_close or yesterday_close
                if price_change_base is not None:
                    try:
                        yc = float(price_change_base)
                        if yc > 0:
    ...
    def _send_notifications(
        ...
        try:
            ...
            if self.notifier.is_available():
                channels = [ch for ch in self.notifier.get_available_channels() if ch != NotificationChannel.TELEGRAM]
