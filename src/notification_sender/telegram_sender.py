...
3. 通过 Telegram Bot API 发送 Markdown 文件
"""
import logging
import mimetypes
...
_BLOCKED_TELEGRAM_DETAIL_KEYWORDS = (
    '🎯 20',
    '决策仪表盘',
    '重要信息速览',
    '数据透视',
...
class TelegramSender:
    ...
    @staticmethod
    def is_detail_content_blocked_for_telegram(content: str) -> bool:
        text = content or ''
        if len(text) > 1500:
            return True
    ...
    def send_to_telegram(self, content: str) -> bool:
        ...
        if self.is_detail_content_blocked_for_telegram(content):
            logger.warning("Telegram 正文发送被保护层拦截：疑似 detailed report")
            return False
    ...
    def _send_telegram_chunked(self, api_url: str, chat_id: str, content: str, max_length: int, message_thread_id: Optional[str] = None) -> bool:
        """分段发送长 Telegram 消息（已禁用）"""
        logger.warning("Telegram 长文本分块发送已全局禁用，避免再次发送原始详细报告")
        return False

    def send_telegram_document(self, file_path: str, caption: str = "") -> bool:
        """Send a local document via Telegram's sendDocument API."""
        if not self._is_telegram_configured():
            return False
        bot_token = self._telegram_config['bot_token']
        chat_id = self._telegram_config['chat_id']
