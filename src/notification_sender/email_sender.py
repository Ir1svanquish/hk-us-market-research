...
class EmailSender:
    ...
    def __init__(self, config: Config):
        ...
        self._market_review_region = (getattr(config, 'market_review_region', 'cn') or 'cn').lower()
    ...
    @staticmethod
    def _subject_prefix_for_region(region: str) -> str:
        region_key = (region or 'cn').lower()
        if region_key == 'us':
            return '美股复盘'
        if region_key == 'hk':
            return '港股复盘'
    ...
    def _build_default_subject(self, region: Optional[str] = None) -> str:
        """Keep email titles consistent across all sending paths."""
        prefix = self._subject_prefix_for_region(region or self._market_review_region)
        return f"{prefix} - {datetime.now().strftime('%Y-%m-%d')}"
    
    def send_to_email(
        self, content: str, subject: Optional[str] = None, receivers: Optional[List[str]] = None,
        attachment_path: Optional[str] = None, attachment_name: Optional[str] = None,
        ...
        try:
            # 生成主题
            if subject is None:
                subject = self._build_default_subject()
            ...
            if high_importance:
                # Widely recognised priority headers used by Outlook, Apple Mail,
                # Gmail-compatible clients and most SMTP relays.
                msg['Importance'] = 'high'
                msg['Priority'] = 'urgent'
