...
def run_full_analysis(
    ...
    try:
        ...
        telegram_merge_enabled = bool(
            getattr(config, 'telegram_bot_token', None)
            and getattr(config, 'telegram_chat_id', None)
        )
        merge_notification = (
            (getattr(config, 'merge_email_notification', False) or telegram_merge_enabled)
        ...
        if merge_notification and (results or market_report) and (
            ...
            if parts:
                ...
                if pipeline.notifier.is_available():
                    used_compact_formatter = False
                    try:
                        from src.telegram_brief_formatter import format_telegram_brief
                        from src.compact_report_formatter import format_compact_report

                        region = getattr(config, 'market_review_region', 'cn') or 'cn'
                        brief_content = format_telegram_brief(results or [], market_report=market_report, region=region)
                        raw_appendix_parts = []
                        report_date = datetime.now().strftime('%Y%m%d')
                        report_md_path = Path(__file__).resolve().parent / 'reports' / f'report_{report_date}_{region}.md'
                        market_md_path = Path(__file__).resolve().parent / 'reports' / f'market_review_{report_date}_{region}.md'
                        if market_md_path.exists():
                            raw_appendix_parts.append(market_md_path.read_text(encoding='utf-8'))
                        if report_md_path.exists():
                            raw_appendix_parts.append(report_md_path.read_text(encoding='utf-8'))
                        raw_appendix = '\n\n---\n\n'.join(part for part in raw_appendix_parts if part.strip())
                        if not raw_appendix.strip():
                            logger.warning('original detailed report is empty')
                            raw_appendix = 'original detailed report is empty'
                        compact_content = format_compact_report(
                            results or [],
                            market_report=market_report,
                            region=region,
                            appendix=raw_appendix,
                        )
                        compact_filename = f"compact_report_{datetime.now().strftime('%Y%m%d')}_{region}.md"
                        compact_filepath = pipeline.notifier.save_report_to_file(compact_content, compact_filename)
                        logger.info('compact report rows: results=%s top=%s others=%s', len(results or []), min(5, len(results or [])), max(0, len(results or []) - min(5, len(results or []))))
                        pdf_filepath = None
                        try:
                            from src.pdf_report import markdown_to_pdf
                            pdf_filename = f"compact_report_{datetime.now().strftime('%Y%m%d')}_{region}.pdf"
                            pdf_filepath = str((Path(compact_filepath).resolve().parent / pdf_filename))
                            markdown_to_pdf(compact_content, pdf_filepath)
                        ...
                        if analysis_stage_only:
                            ...
                            if email_available:
                                email_subject = pipeline.notifier._build_default_subject(region=region)
                                email_sent = pipeline.notifier.send_to_email(
                                    brief_content,
                                    subject=email_subject,
                                    receivers=pipeline.notifier.get_all_email_receivers(),
                                    attachment_path=pdf_filepath,
                                    attachment_name=Path(pdf_filepath).name if pdf_filepath else None,
                                )

                            other_sent = pipeline.notifier.send(combined_content, email_send_to_all=True, skip_channels=['telegram', 'email'])
                            if telegram_sent or email_sent or other_sent:
                                used_compact_formatter = True
...
def main() -> int:
    ...
    try:
        ...
        force_one_shot_mode = os.getenv('FORCE_ONE_SHOT_MODE', 'false').lower() == 'true'
        if force_one_shot_mode:
            logger.info('FORCE_ONE_SHOT_MODE=true，禁止进入定时任务模式')

        # 模式2: 定时任务模式
        if (args.schedule or config.schedule_enabled) and not force_one_shot_mode:
            logger.info("模式: 定时任务")
