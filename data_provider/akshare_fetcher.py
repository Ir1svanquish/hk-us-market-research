...
# 港股实时行情缓存（与A股 _realtime_cache 对齐，解决 N+1 全量拉取问题）
_hk_realtime_cache: Dict[str, Any] = {
    'data': None,
    'timestamp': 0,
    'ttl': 1200,  # 20分钟缓存有效期
    'source': None,  # 'em' 或 'sina'
}
...
def _call_with_timeout(seconds: int, fn, *args, **kwargs):
    """在独立线程中执行函数并设置超时，防止 requests timeout=None 导致无限阻塞。"""
...
class AkshareFetcher(BaseFetcher):
    ...
    def _get_hk_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        缓存机制（与 A 股 _realtime_cache 对齐）：
        - 第一次调用拉取全市场数据并缓存，TTL 20 分钟
        - 后续股票直接从缓存查找，避免 N+1 全量拉取
        - 超时保护：单次 API 调用最多等待 90s，防止无限阻塞
        """
        import akshare as ak
        circuit_breaker = get_realtime_circuit_breaker()
        ...
        if (_hk_realtime_cache['data'] is not None and
            ...
            cached_source = _hk_realtime_cache['source']
            cache_age = int(current_time - _hk_realtime_cache['timestamp'])
            logger.debug(f"[缓存命中] 港股实时行情({cached_source}) - 缓存年龄 {cache_age}s/{_hk_realtime_cache['ttl']}s")
            return self._extract_hk_quote_from_df(df, code, stock_code, cached_source)
        ...
        if circuit_breaker.is_available(em_key):
            try:
                ...
                api_start = time.time()
                df = _call_with_timeout(_HK_SPOT_TIMEOUT, ak.stock_hk_spot_em)
                api_elapsed = time.time() - api_start
        ...
        # --- 备用数据源：新浪 ---
        if df is None and circuit_breaker.is_available(sina_key):
            try:
                logger.info(f"[API调用] ak.stock_hk_spot() 获取港股实时行情（备用）...")
                api_start = time.time()
                df = _call_with_timeout(_HK_SPOT_TIMEOUT, ak.stock_hk_spot)
                api_elapsed = time.time() - api_start
        ...
        if df is not None and not df.empty:
            ...
            _hk_realtime_cache['source'] = data_source
            logger.info(f"[缓存更新] 港股实时行情({data_source}) 缓存已刷新，TTL={_hk_realtime_cache['ttl']}s")
        else:
            logger.info(f"[实时行情] 港股实时行情获取失败，所有数据源不可用")
            return None

        return self._extract_hk_quote_from_df(df, code, stock_code, data_source)

    def _extract_hk_quote_from_df(
        self, df: 'pd.DataFrame', code: str, stock_code: str, source: str
    ) -> Optional[UnifiedRealtimeQuote]:
        """从缓存的港股全量 DataFrame 中提取单只股票行情。"""
        row = df[df['代码'] == code]
        if row.empty:
            logger.info(f"[API返回] 未找到港股 {code} 的实时行情 ({source})")
            return None
