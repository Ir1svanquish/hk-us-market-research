...
class DataFetcherManager:
    ...
    def get_fundamental_context(
        ...
        if market == "us":
            return self._build_us_fundamental_context(stock_code)
        ...
        if market == "hk":
            if not getattr(config, "enable_hk_fundamental_context", True):
                return self._build_market_not_supported(
                    market="hk",
                    reason="hk fundamental context disabled",
                )
            hk_context = self._build_hk_fundamental_context(stock_code)
            if stage_timeout > 0 and self._should_cache_fundamental_context(hk_context):
    ...
    def _build_us_fundamental_context(self, stock_code: str) -> Dict[str, Any]:
        """美股最小基本面增强：Finnhub 财报/推荐趋势。"""
        config = self._get_fundamental_config()
        service = FinnhubService(
            api_keys=getattr(config, "finnhub_api_keys", []),
            timeout=float(getattr(config, "finnhub_timeout_seconds", 8.0)),
        )
        result_ctx: Dict[str, Any] = {
            "market": "us",
            "valuation": {},
        ...
        if not service.is_available:
            earnings_block = self._build_fundamental_block(
                "not_supported",
                {},
                [{"provider": "finnhub", "result": "not_supported", "duration_ms": 0}],
                ["finnhub api key missing"],
            )
            institution_block = self._build_fundamental_block(
                "not_supported",
                {},
                [{"provider": "finnhub", "result": "not_supported", "duration_ms": 0}],
                ["finnhub api key missing"],
            ...
            timeout_budget = max(
                float(getattr(config, "finnhub_timeout_seconds", 8.0)),
                min(
                    float(getattr(config, "fundamental_fetch_timeout_seconds", 0.8)),
                    max(0.1, float(getattr(config, "fundamental_stage_timeout_seconds", 1.5))),
                ),
            )
            payload, err, duration_ms = self._run_with_retry(
                lambda: service.get_us_earnings_context(stock_code),
                timeout_budget,
                "finnhub_us_fundamental",
            )
            if not isinstance(payload, dict):
                payload = {
                    "status": "failed",
                    "data": {},
                    "errors": [err or "finnhub us fundamental failed"],
                    "source_chain": [{"provider": "finnhub", "result": "failed", "duration_ms": duration_ms}],
                }
            ...
            institution_payload = {
                "finnhub_recommendation": data.get("recommendation_trend", {}),
            }
            earnings_block = self._build_fundamental_block(
                self._infer_block_status(earnings_payload, str(payload.get("status", "partial"))),
                earnings_payload,
                source_chain,
                errors,
            )
            institution_block = self._build_fundamental_block(
                self._infer_block_status(institution_payload, str(payload.get("status", "partial"))),
                institution_payload,
                source_chain,
                errors,
            )
        ...
        result_ctx["valuation"] = self._build_fundamental_block(
            ...
            {},
            [{"provider": "fundamental_pipeline", "result": "not_supported", "duration_ms": 0}],
            ["us valuation handled by realtime quote"],
        )
        result_ctx["growth"] = self._build_fundamental_block(
            "not_supported",
            {},
            [{"provider": "fundamental_pipeline", "result": "not_supported", "duration_ms": 0}],
            ["us growth not yet integrated"],
        )
        result_ctx["earnings"] = earnings_block
        result_ctx["institution"] = institution_block
        result_ctx["capital_flow"] = self._build_fundamental_block(
            "not_supported",
            {},
            [{"provider": "fundamental_pipeline", "result": "not_supported", "duration_ms": 0}],
            ["us capital flow not supported"],
        )
        result_ctx["dragon_tiger"] = self._build_fundamental_block(
            "not_supported",
            {},
            [{"provider": "fundamental_pipeline", "result": "not_supported", "duration_ms": 0}],
            ["us dragon tiger not supported"],
        )
        result_ctx["boards"] = self._build_fundamental_block(
            "not_supported",
            {},
            [{"provider": "fundamental_pipeline", "result": "not_supported", "duration_ms": 0}],
            ["us boards not supported"],
        )
        ...
        block_statuses = list(result_ctx["coverage"].values())
        if all(v == "not_supported" for v in block_statuses):
            result_ctx["status"] = "not_supported"
        elif any(v in {"ok", "partial"} for v in block_statuses):
            result_ctx["status"] = "partial" if any(v == "partial" for v in block_statuses) else "ok"
        else:
            result_ctx["status"] = "failed"
        result_ctx["elapsed_ms"] = int((time.time() - start_ts) * 1000)
        return result_ctx

    def _build_hk_fundamental_context(self, stock_code: str) -> Dict[str, Any]:
        """港股基础资料 + 历史衍生字段 + 港股通持股。"""
        result_ctx: Dict[str, Any] = {
            "market": "hk",
            "valuation": {},
        ...
        if tushare_fetcher is None:
            return self._build_market_not_supported("hk", "tushare fetcher unavailable")

        profile = self._call_fetcher_method(tushare_fetcher, "get_hk_basic_profile", stock_code)
        hold_summary = self._call_fetcher_method(tushare_fetcher, "get_hk_hold_summary", stock_code)

        try:
            end_date = datetime.now()
            hist_df = self._call_fetcher_method(
                tushare_fetcher,
                "fetch_data",
                stock_code,
                (end_date - timedelta(days=180)).strftime("%Y-%m-%d"),
                (end_date - timedelta(days=1)).strftime("%Y-%m-%d"),
            )
        except Exception as e:
            hist_df = pd.DataFrame()
            result_ctx["errors"].append(f"hk_history:{e}")

        history_payload: Dict[str, Any] = {}
        if isinstance(hist_df, pd.DataFrame) and not hist_df.empty:
            work = hist_df.sort_values("date").copy()
            latest = work.iloc[-1]
            if len(work) >= 2:
                prev_close = float(work.iloc[-2].get("close") or 0) or None
            else:
                prev_close = None
            latest_close = float(latest.get("close") or 0) or None
            if latest_close is not None:
                history_payload["latest_close"] = round(latest_close, 4)
            if prev_close and latest_close:
                history_payload["day_change_pct"] = round((latest_close / prev_close - 1) * 100, 4)
            if len(work) >= 60:
                close_60 = float(work.iloc[-60].get("close") or 0) or None
                if close_60 and latest_close:
                    history_payload["change_60d"] = round((latest_close / close_60 - 1) * 100, 4)
            window = work.tail(60)
            if not window.empty:
                history_payload["range_60d_high"] = float(window["high"].max()) if "high" in window else None
                history_payload["range_60d_low"] = float(window["low"].min()) if "low" in window else None
                history_payload["avg_amount_20d"] = float(work.tail(20)["amount"].mean()) if "amount" in work else None

        valuation_payload = {
            "change_60d": history_payload.get("change_60d"),
        }
        result_ctx["valuation"] = self._build_fundamental_block(
            self._infer_block_status(valuation_payload, "partial"),
            valuation_payload,
            [{"provider": "tushare_hk_daily", "result": "partial", "duration_ms": 0}],
            [],
        )

        growth_payload: Dict[str, Any] = {}
        earnings_payload: Dict[str, Any] = {
            "hk_profile": profile or {},
            "hk_history_metrics": history_payload or {},
        }
        institution_payload: Dict[str, Any] = {
            "hk_connect_holding": hold_summary or {},
        }

        result_ctx["growth"] = self._build_fundamental_block(
            self._infer_block_status(growth_payload, "not_supported"),
            growth_payload,
            [{"provider": "tushare_hk_basic", "result": "not_supported", "duration_ms": 0}],
            [],
        )
        result_ctx["earnings"] = self._build_fundamental_block(
            self._infer_block_status(earnings_payload, "partial"),
            earnings_payload,
            [
                {"provider": "tushare_hk_basic", "result": "ok" if profile else "partial", "duration_ms": 0},
                {"provider": "tushare_hk_daily", "result": "ok" if history_payload else "partial", "duration_ms": 0},
            ],
            [],
        )
        result_ctx["institution"] = self._build_fundamental_block(
            self._infer_block_status(institution_payload, "partial"),
            institution_payload,
            [{"provider": "tushare_hk_hold", "result": "ok" if hold_summary else "partial", "duration_ms": 0}],
            [],
        )
        result_ctx["capital_flow"] = self._build_fundamental_block(
            "not_supported",
        ...
        result_ctx["boards"] = self.get_board_context(stock_code, budget_seconds=0.5)
