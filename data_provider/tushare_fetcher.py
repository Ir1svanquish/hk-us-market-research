...
class TushareFetcher(BaseFetcher):
    ...
    def get_hk_basic_profile(self, stock_code: str) -> Dict[str, Any]:
        """获取港股基础资料。"""
        if self._api is None or not _is_hk_market(stock_code):
            return {}
        try:
            self._check_rate_limit()
            ts_code = self._convert_hk_stock_code_for_tushare(stock_code)
            df = self._api.hk_basic(
                ts_code=ts_code,
                fields='ts_code,name,list_date,trade_unit,isin,curr_type'
            )
            if df is None or df.empty:
                return {}
            row = df.iloc[0]
            return {
                ...
                'currency': str(row.get('curr_type', '') or ''),
            }
        except Exception as e:
            logger.warning(f"Tushare 获取港股基础资料失败 {stock_code}: {e}")
            return {}

    def get_hk_hold_summary(self, stock_code: str) -> Dict[str, Any]:
        """获取港股通持股摘要。"""
        if self._api is None or not _is_hk_market(stock_code):
            return {}
        try:
            self._check_rate_limit()
            ts_code = self._convert_hk_stock_code_for_tushare(stock_code)
            df = self._api.hk_hold(ts_code=ts_code, limit=5)
            if df is None or df.empty:
                return {}
            work = df.copy()
            ...
            return {
                ...
                'hold_vol': row.get('vol'),
                'hold_ratio': row.get('ratio'),
                'exchange': str(row.get('exchange', '') or ''),
            }
        except Exception as e:
            logger.warning(f"Tushare 获取港股通持股失败 {stock_code}: {e}")
            return {}
