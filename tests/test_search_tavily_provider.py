...
class _FakeTavilyClient:
    ...
    def search(self, **kwargs):
        ...
        if isinstance(type(self).response_payload, Exception):
            raise type(self).response_payload
...
class TestTavilySearchProvider(unittest.TestCase):
    ...
    def test_invalid_key_is_persistently_quarantined_in_news_mode(self) -> None:
        provider = TavilySearchProvider(["dummy_key"])

        with self._patch_tavily(RuntimeError("Unauthorized: missing or invalid API key")), \
             patch('src.search_service.quarantine_api_key') as quarantine_mock, \
             patch('src.search_service.record_api_result'), \
             patch('src.search_service.is_cooled_down', return_value=False):
            resp = provider.search("BABA latest news", max_results=3, topic="news")

        self.assertFalse(resp.success)
        self.assertIn("已持久隔离", resp.error_message)
    ...
    def test_news_mode_success_uses_shared_usage_and_probe_tracking(self) -> None:
        provider = TavilySearchProvider(["dummy_key"])

        with self._patch_tavily({"results": []}), \
             patch('src.search_service.record_api_result') as usage_mock, \
             patch('src.search_service.reset_probe_failure') as reset_mock, \
             patch('src.search_service.is_cooled_down', return_value=False):
            resp = provider.search("BABA latest news", max_results=3, topic="news")

        self.assertTrue(resp.success)
        usage_mock.assert_called_once_with("Tavily", "dummy_key", success=True)
        reset_mock.assert_called_once_with("Tavily", "dummy_key")
