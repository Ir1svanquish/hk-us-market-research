...
class LLMChannelConfigTestCase(unittest.TestCase):
    ...
    @patch("src.config.setup_env")
    @patch.object(Config, "_parse_litellm_yaml", return_value=[])
    def test_channel_reads_api_key_from_protected_file(self, _mock_parse_yaml, _mock_setup_env) -> None:
        with TemporaryDirectory() as temp_dir:
            key_file = Path(temp_dir) / "provider.key"
            key_file.write_text("sk-file-test-value\n", encoding="utf-8")
            env = {
                "LLM_CHANNELS": "primary",
                "LLM_PRIMARY_PROTOCOL": "openai",
                "LLM_PRIMARY_BASE_URL": "https://api.example.com/v1",
                "LLM_PRIMARY_API_KEY_FILE": str(key_file),
                "LLM_PRIMARY_MODELS": "example-model",
            }

            with patch.dict(os.environ, env, clear=True):
                config = Config._load_from_env()

        self.assertEqual(config.llm_channels[0]["api_keys"], ["sk-file-test-value"])
        self.assertEqual(
            config.llm_model_list[0]["litellm_params"]["api_key"],
            "sk-file-test-value",
        )

    @patch("src.config.setup_env")
    @patch.object(Config, "_parse_litellm_yaml", return_value=[])
    def test_social_sentiment_reads_api_key_from_protected_file(
        self,
        _mock_parse_yaml,
        _mock_setup_env,
    ) -> None:
        with TemporaryDirectory() as temp_dir:
            key_file = Path(temp_dir) / "adanos.key"
            key_file.write_text("sk_live_file_test\n", encoding="utf-8")
            env = {"SOCIAL_SENTIMENT_API_KEY_FILE": str(key_file)}

            with patch.dict(os.environ, env, clear=True):
                config = Config._load_from_env()

        self.assertEqual(config.social_sentiment_api_key, "sk_live_file_test")
