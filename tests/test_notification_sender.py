...
class TestEmailSender(unittest.TestCase):
    ...
    @mock.patch("smtplib.SMTP_SSL")
    def test_send_to_email_marks_high_importance(self, mock_smtp_ssl):
        cfg = _config(
            email_sender="a@qq.com",
            email_password="p",
            email_receivers=["b@qq.com"],
    ...
    def test_build_default_subject_uses_market_region(self):
        us_sender = EmailSender(
            _config(
                email_sender="a@qq.com",
                email_password="p",
        ...
        self.assertTrue(us_sender._build_default_subject().startswith("美股复盘 - "))
        self.assertTrue(hk_sender._build_default_subject().startswith("港股复盘 - "))
...
class TestTelegramSender(unittest.TestCase):
    ...
    @mock.patch("src.notification_sender.telegram_sender.requests.post")
    def test_send_pdf_document_uses_pdf_mime_type(self, mock_post):
        mock_post.return_value = _response(200, {"ok": True})
        cfg = _config(telegram_bot_token="BOT", telegram_chat_id="CHAT")
        sender = TelegramSender(cfg)
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(sender.send_telegram_document(path, caption="paired report"))
        document = mock_post.call_args[1]["files"]["document"]
        self.assertEqual(document[0], "report.pdf")
        self.assertEqual(document[2], "application/pdf")
