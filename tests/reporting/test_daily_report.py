...
class DailyReportTests(unittest.TestCase):
    def test_us_digest_hides_internal_theme_placeholder_and_names_risk_stock(self) -> None:
        ...
        _subject, body = build_report_digest(
            contract,
            {"markets": {"us": {"reporting_rolling_20": {}}}},
            "us",
            "2026-08-28",
    ...
    def test_comparison_message_contains_pairing_progress_and_replacements(self) -> None:
        summary = {
            "markets": {
                "us": {
                    ...
                    "comparison": {
                        "paired_periods_5d": 2,
                        "analysis_average_1d": 1.0,
                        "reporting_average_1d": 1.5,
                    },
                    "quality_verdict": {"label": "证据不足"},
                    "latest_reporting_run": {
                        "analysis_top5_json": '["OLD", "SAME"]',
                        "overlap_ratio": 50,
        ...
        contract = {
            "stocks": [
                {
                    "identity": {"market": "us", "symbol": "NEW", "name": "报告入选"},
                ...
                {
                    "identity": {"market": "us", "symbol": "OLD", "name": "分析阶段未入选"},
                    "opportunity": {"rank": 8, "score": 50},
                    "execution": {"status": "研究关注"},
                },
            ]
        }
        text = build_validation_summary(summary, contract, "us", "2026-08-26")
        self.assertIn("5日配对 2/40期", text)
        self.assertIn("报告入选：报告入选", text)
        self.assertIn("未入选：分析阶段未入选", text)
        self.assertIn("8–12周", text)
        self.assertLessEqual(len(text), 1490)

    def test_report_digest_uses_reader_facing_structure_and_dated_subject(self) -> None:
        contract = {
            "stocks": [
                {
        ...
        subject, body = build_report_digest(
            contract,
            validation,
            "hk",
            "2026-08-27",
        ...
        telegram = build_telegram_digest(
            subject,
            contract,
            validation,
            "hk",
    ...
    def test_report_delivery_sends_only_formal_pdf_and_digest(self, service_cls) -> None:
        contract = {
            "stocks": [
                {
                    "identity": {"market": "hk", "symbol": "HK00700", "name": "腾讯控股"},
        ...
        validation = {"markets": {"hk": {"reporting_rolling_20": {"sample_5d": 0}}}}
        hk_variables = {
            "signal": {"label": "中性", "summary": "南向小幅净流入。"},
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            analysis_pdf.write_bytes(b"analysis")
            report_pdf.write_bytes(b"report")

            state = deliver_report(
                {
        ...
        self.assertFalse(state["analysis_stage_external_delivery"])
        notifier.send_telegram_document.assert_called_once_with(
            str(report_pdf), caption="港股复盘及机会日报｜2026-08-27｜中性"
        )
        email_kwargs = notifier.send_to_email.call_args.kwargs
        self.assertEqual(email_kwargs["attachment_path"], str(report_pdf))
        self.assertNotEqual(email_kwargs["attachment_path"], str(analysis_pdf))
