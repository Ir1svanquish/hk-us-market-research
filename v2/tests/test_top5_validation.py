...
class Top5ValidationTests(unittest.TestCase):
    ...
    def test_legacy_top5_tracks_forward_returns_and_excursions(self) -> None:
        ...
        self.assertEqual(self.ledger.import_legacy_history(state), 1)
    ...
    def test_structured_breakout_records_trigger_fill_target_and_next_session_return(self) -> None:
        ...
        row = dict(
            self.ledger.connection.execute(
                "SELECT * FROM opportunity_validation_snapshots WHERE source_kind='shadow_v2'"
            ).fetchone()
        )
    ...
    def test_weight_changes_remain_blocked_before_twenty_periods(self) -> None:
        ...
        self.assertEqual(summary["promotion_verdict"]["status"], "insufficient_evidence")

    def test_promotion_verdict_requires_positive_interval_and_non_worse_risk(self) -> None:
        verdict = self.ledger._promotion_verdict(
            {"paired_periods_5d": 40, "paired_delta_5d_ci90": [0.2, 1.4]},
            {"average_mae": -8.0},
            {"average_mae": -7.5},
            {"ready_to_adjust": True, "matured_rows_5d": 100},
        )
        self.assertEqual(verdict["status"], "shadow_better")

        worse_risk = self.ledger._promotion_verdict(
            {"paired_periods_5d": 40, "paired_delta_5d_ci90": [0.2, 1.4]},
            {"average_mae": -8.0},
            {"average_mae": -9.0},
            {"ready_to_adjust": True, "matured_rows_5d": 100},
        )
        self.assertEqual(worse_risk["status"], "shadow_worse")

        preliminary = self.ledger._promotion_verdict(
