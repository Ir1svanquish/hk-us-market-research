"""Point-in-time Top5 validation and shadow-run accuracy ledger.

The production Top5 history is imported as a return baseline.  Structured V2
contracts are stored separately so trigger, stop/target and score-component
statistics never get mixed with the legacy ranking.  All writes are idempotent
...
DEFAULT_LEDGER = V2_ROOT / "state" / "decision_accuracy.sqlite3"
DEFAULT_LEGACY_STATE = PROJECT_ROOT / "state" / "top5_brief_stats.json"
VALIDATION_VERSION = "top5-validation-v1"
LEGACY_MODEL_VERSION = "legacy-production-top5"
HORIZONS = (1, 5, 20)
...
class Top5ValidationLedger:
    """SQLite ledger for production baselines and V2 shadow signals."""
    ...
    def _create_schema(self) -> None:
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS shadow_run_audits (
    ...
    def import_legacy_history(self, path: Path = DEFAULT_LEGACY_STATE) -> int:
            ...
            for entry in node.get("entries") or []:
                ...
                for rank, symbol in enumerate(entry.get("codes") or [], 1):
                    ...
                    self._upsert_snapshot(
                        {
                            "record_id": _record_id(as_of, market, symbol, "production", LEGACY_MODEL_VERSION),
                            "as_of": as_of,
                            "market": market,
                            "symbol": symbol,
                            "name": symbol,
    ...
    def record_contract(self, contract: Mapping[str, Any]) -> int:
        ...
        for item in contract.get("stocks") or []:
            ...
            self._upsert_snapshot(
                {
                    "record_id": _record_id(as_of, market, symbol, "shadow_v2", scoring_version),
                    "as_of": as_of,
                    "market": market,
                    "symbol": symbol,
                    "name": str(identity.get("name") or symbol),
                    "source_kind": "shadow_v2",
        ...
        for market, items in by_market.items():
            self._record_shadow_run(contract, market, items)
        return rows

    def _production_top5(self, as_of: str, market: str) -> list[str]:
        return [
            str(row[0])
            for row in self.connection.execute(
                """
                SELECT symbol FROM opportunity_validation_snapshots
                WHERE as_of=? AND market=? AND source_kind='production' AND is_top5=1
                ORDER BY rank
                """,
                (as_of, market),
            )
    ...
    def _record_shadow_run(
        ...
        production = self._production_top5(as_of, market)
        overlap = len(set(top5) & set(production)) if production else None
        ...
        self.connection.execute(
            """
            INSERT INTO shadow_run_audits (
                run_id,as_of,market,model_version,pool_size,top5_json,production_top5_json,
            ...
            ON CONFLICT(as_of,market,model_version) DO UPDATE SET
                ...
                production_top5_json=excluded.production_top5_json,
            ...
            """,
            (
                run_id, as_of, market, model, pool_size, _json(top5), _json(production),
    ...
    def update_report_audit(self, as_of: str, market: str, report_result: Mapping[str, Any]) -> None:
        self.connection.execute(
            """
            UPDATE shadow_run_audits SET report_layout_ok=?,report_content_ok=?,report_pages=?,
    ...
    def evaluate(self, market_db: Path = DEFAULT_DB, through_date: str | None = None) -> int:
        ...
        for snapshot in snapshots:
            ...
            if item.get("source_kind") == "shadow_v2" and item.get("trigger_active") and item.get("trigger_price"):
    ...
    def _weight_validation(self, market: str) -> dict[str, Any]:
        rows = self._rows(market, "shadow_v2", top5_only=False)
    ...
    def _comparison(self, market: str) -> dict[str, Any]:
        runs = [dict(row) for row in self.connection.execute(
            "SELECT * FROM shadow_run_audits WHERE market=? ORDER BY as_of DESC LIMIT 20", (market,)
        )]
        comparable = [row for row in runs if row.get("overlap_count") is not None]
        production = { (row["as_of"], row["symbol"]): row for row in self._rows(market, "production") }
        shadow = { (row["as_of"], row["symbol"]): row for row in self._rows(market, "shadow_v2") }
        result: dict[str, Any] = {
        ...
        for horizon in HORIZONS:
            ...
            result[f"production_average_{horizon}d"] = round(mean(prod_values), 2) if prod_values else None
            result[f"shadow_average_{horizon}d"] = round(mean(shadow_values), 2) if shadow_values else None
            paired_deltas: list[float] = []
            for day in sorted(comparable_dates):
                production_day = [
                    _number(row.get(f"return_{horizon}d"))
                    for (row_day, _symbol), row in production.items()
                    if row_day == day
                ]
                shadow_day = [
                    _number(row.get(f"return_{horizon}d"))
                    for (row_day, _symbol), row in shadow.items()
                    if row_day == day
                ]
                production_valid = [value for value in production_day if value is not None]
                shadow_valid = [value for value in shadow_day if value is not None]
                if production_valid and shadow_valid:
                    paired_deltas.append(mean(shadow_valid) - mean(production_valid))
        ...
        if not required or not isinstance(ci, list) or len(ci) != 2:
            ...
            return {
                ...
                "reason": (
                    ...
                    f"（前20期研究+后20期盲测）；成熟影子样本 "
                    f"{weight.get('matured_rows_5d', 0)}/100"
                ),
            }
        production_mae = _number(production.get("average_mae"))
        shadow_mae = _number(shadow.get("average_mae"))
        risk_not_worse = (
            production_mae is None
            or shadow_mae is None
            or shadow_mae >= production_mae
        )
        if _number(ci[0]) is not None and float(ci[0]) > 0 and risk_not_worse:
            return {
                "status": "shadow_better",
                "label": "新版更优",
    ...
    def build_market_summary(self, market: str, through_date: str) -> dict[str, Any]:
        production = self._rows(market, "production")
        shadow = self._rows(market, "shadow_v2")
        baseline_rows = self._last_periods(production, 20)
        shadow_rows = self._last_periods(shadow, 20)
        completed_dates = sorted({row["as_of"] for row in production if row.get("return_1d") is not None})
        latest_date = completed_dates[-1] if completed_dates else ""
        latest = [row for row in production if row["as_of"] == latest_date]
        latest_run_row = self.connection.execute(
            "SELECT * FROM shadow_run_audits WHERE market=? AND as_of<=? ORDER BY as_of DESC LIMIT 1",
            (market, through_date),
        ).fetchone()
        ...
        shadow_metrics = {
            "periods": len({row["as_of"] for row in shadow_rows}),
            **self._metric_summary(shadow_rows),
        }
        ...
        return {
            ...
            "shadow_rolling_20": shadow_metrics,
            "by_score_band": self._group_metrics(shadow_rows, "score_band"),
            "by_signal_type": self._group_metrics(shadow_rows, "signal_type"),
            "comparison": comparison,
            "weight_validation": weight_validation,
            "promotion_verdict": self._promotion_verdict(
                comparison, rolling_metrics, shadow_metrics, weight_validation
            ),
            "latest_shadow_run": latest_run,
        }
...
def render_markdown(summary: Mapping[str, Any]) -> str:
    lines = [f"# Top5验证与影子准确率账本｜{summary.get('through_date')}", ""]
    for market, label in (("hk", "港股"), ("us", "美股")):
        ...
        lines.extend(
            [
                ...
                f"- V2影子：{shadow.get('periods', 0)}期 / {shadow.get('signals', 0)}条；触发 {shadow.get('triggered', 0)}/{shadow.get('trigger_eligible', 0)}。",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build Top5 validation and shadow-run ledger")
    parser.add_argument("--db", type=Path, default=DEFAULT_DB)
    parser.add_argument("--ledger", type=Path, default=DEFAULT_LEDGER)
    parser.add_argument("--legacy-state", type=Path, default=DEFAULT_LEGACY_STATE)
    ...
    contracts = args.contract or sorted((V2_ROOT / "output").glob("full_pool_contract_*.json"))
    ledger = Top5ValidationLedger(args.ledger)
    legacy_rows = ledger.import_legacy_history(args.legacy_state)
    contract_rows = 0
    ...
    summary["ingest"] = {"legacy_rows": legacy_rows, "contract_rows": contract_rows, "evaluated_rows": evaluated}
    output_json = args.output_json or V2_ROOT / "output" / f"top5_validation_{args.through_date.replace('-', '')}.json"
