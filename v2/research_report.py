"""Render an isolated research-enhanced edition of the legacy daily report.

The report keeps the expensive, decision-relevant legacy inputs (market/sector
...
removing repeated prose.  It optionally overlays the already-generated V2
quality/event/trade-card review for the covered symbols.

This module is deliberately read-only with respect to the production chain:
it reads the analysis database and existing report artifacts, and writes only
to ``v2/output`` (or explicit output paths).
"""
...
DEFAULT_V2_REVIEW = V2_ROOT / "output" / "event_context_top10_trading_cards_20260821.json"
...
def _load_v2_review(path: Path | None, market: str) -> dict[str, dict[str, Any]]:
...
def load_stocks(
    ...
    report_date: str,
    market: str,
    v2_review_path: Path | None = DEFAULT_V2_REVIEW,
) -> list[ResearchStock]:
    ...
    try:
        ...
        evidence_by_symbol = {}
    review = _load_v2_review(v2_review_path, market)
    ...
    for row in rows:
        ...
        stocks.append(
            ResearchStock(
                ...
                v2=review.get(_symbol_key(symbol, market), {}),
...
def _v2_overlay(item: ResearchStock) -> str:
    if not item.v2:
        return '<div class="v2-empty">V2 官方事件/质量复核尚未覆盖本标的；保留旧版结论，正式启用前需补跑。</div>'
    card = _as_dict(item.v2.get("trade_card"))
    gate = _as_dict(item.v2.get("final_gate"))
    ...
    return f"""
      <div class="v2-grid">
        <div><small>V2最终动作</small><b>{_e(item.v2.get('final_action') or card.get('label') or '—')}</b></div>
        <div><small>数据完整度</small><b>{_e(item.v2.get('data_completeness', '—'))}</b></div>
        <div><small>信号置信度</small><b>{_e(item.v2.get('signal_confidence', '—'))}</b></div>
...
def _active_page(item: ResearchStock, report_date: str) -> str:
    ...
    <section class="page stock-page">
      ...
      <div class="v2"><h3>新增：V2 质量、事件与执行复核</h3>{_v2_overlay(item)}</div>
...
def _watch_card(item: ResearchStock) -> str:
    ...
    v2_action = item.v2.get("final_action") if item.v2 else "待补跑"
    return f"""
    <article class="watch-card">
      ...
      <div class="watch-foot"><span>{_status_badges(item)}</span><span>V2复核：{_e(v2_action)}</span></div>
...
def _decision_rows(stocks: list[ResearchStock]) -> str:
    rows = []
    for item in stocks:
        v2_action = item.v2.get("final_action") if item.v2 else "待复核"
        rows.append(
            ...
            f"<td>{_e(item.display_completeness)}%</td><td>{_e(v2_action)}</td></tr>"
        )
    return "".join(rows)
...
"""
    ...
    <section class="page">
      ...
      <div class="principles"><div><b>量价不丢</b><p>长桥实时字段与历史相对成交量分开标注，保留来源和时间口径。</p></div><div><b>研究不丢</b><p>板块网页研究、事件证据、美股社媒舆情继续进入决策卡。</p></div><div><b>结论唯一</b><p>旧版动作保留；V2复核另列，发生冲突时不再悄悄覆盖。</p></div></div>
    </section>

    <section class="page"><div class="page-title"><div><span class="eyebrow">市场与板块研究</span><h2>大盘、风格与风险环境</h2></div><span class="count">保留旧版研究精华</span></div>
    ...
    <section class="page"><div class="page-title"><div><span class="eyebrow">决策总表</span><h2>动作、量价与 V2 复核</h2></div><span class="count">{len(stocks)} 只</span></div>
      <div class="legend"><span>相对量＝当日成交量/历史均量口径</span><span>｜</span><span>“待复核”表示 V2 官方事件/质量链尚未覆盖</span></div>
      <table class="decision-table"><thead><tr><th>股票</th><th>旧版动作</th><th>分数</th><th>方向</th><th>现价/涨跌</th><th>相对量</th><th>换手</th><th>字段完整</th><th>V2复核</th></tr></thead><tbody>{_decision_rows(stocks)}</tbody></table>
      <div class="quality-note"><b>新增改进：</b>报告同时显示旧版结论与 V2 复核，不再把事件闸门、完整度或盈亏比校验前后的动作混为一个结论。量比缺失时优先展示数据库中同日相对成交量，并标明计算来源。</div>
    </section>
...
def render_report(
    ...
    stocks = load_stocks(db_path, formal.report_date, market, v2_review_path)
    ...
    return {
        ...
        "v2_reviewed": sum(bool(item.v2) for item in stocks),
