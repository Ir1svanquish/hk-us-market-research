...
This is an isolated V2 preview.  It does not alter the production report or
notification pipeline.  Daily market observations are fetched from primary
sources where practical and every normalized value retains its observation
...
USER_AGENT = "daily-stock-analysis-v2/1.0 key-variables-preview"
HKEX_TEMPLATE = (
...
<header class="hero"><div><span class="eyebrow">HK MARKET DRIVERS · V2 PREVIEW</span><h1>港股关键变量</h1><p>{html.escape(signal['summary'])}</p></div><div class="stance"><span>{payload['as_of']} 收盘后</span><b>{html.escape(signal['label'])}</b><span>只提供市场背景，不改Top5评分</span></div></header>
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Render an isolated HK key-variable PDF preview")
