"""Independent CLI for the V2 official filing metadata experiment."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sync official filing metadata into the isolated V2 store")
    ...
    parser.add_argument("--cache-dir", type=Path, default=V2_ROOT / "cache")
    parser.add_argument("--output", type=Path, default=V2_ROOT / "output" / "last_sync.json")
    ...
    parser.add_argument("--sec-user-agent", default=os.getenv("V2_SEC_USER_AGENT", ""))
    parser.add_argument(
...
def main(argv: List[str] | None = None) -> int:
    ...
    hk_symbols = _split_symbols(args.hk_symbols or [os.getenv("V2_HK_SYMBOLS", "")])
    us_symbols = _split_symbols(args.us_symbols or [os.getenv("V2_US_SYMBOLS", "")])
    if not hk_symbols and not us_symbols:
        raise SystemExit("No symbols supplied; use --hk-symbols/--us-symbols or V2_HK_SYMBOLS/V2_US_SYMBOLS")
    ...
    if not sec_user_agent and args.sec_contact_env_file:
        ...
        if contact:
            sec_user_agent = f"daily-stock-analysis-v2 {contact}"
    if us_symbols and not sec_user_agent:
        raise SystemExit("US sync requires --sec-user-agent or V2_SEC_USER_AGENT")
