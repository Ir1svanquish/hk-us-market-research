...
This module is intentionally isolated from the production report pipeline.  It
uses the structured 2026 report inputs already consumed by ``research_report``
and writes only explicit V2 output artifacts.
"""
...
from v2.official_sources.market_metrics import calculate_history_metrics
...
def _event_score(stock: ResearchStock) -> float:
    ...
    event_gate = stock.v2.get("event_gate") if isinstance(stock.v2.get("event_gate"), dict) else {}
...
def _data_score(stock: ResearchStock) -> float:
    ...
    v2_completeness = stock.v2.get("data_completeness")
    if isinstance(v2_completeness, (int, float)):
        completeness = (completeness + float(v2_completeness)) / 2
    return round(_clamp(completeness / 20, 0, 5), 1)


def _risk_reward(stock: ResearchStock) -> float | None:
    card = stock.v2.get("trade_card") if isinstance(stock.v2.get("trade_card"), dict) else {}
...
def _event_priority(stock: ResearchStock) -> int:
    gate = stock.v2.get("event_gate") if isinstance(stock.v2.get("event_gate"), dict) else {}
...
def _execution(
    ...
    # Structured official/provider events are authoritative.  The legacy text
    ...
    event_days, event_date = (None, "") if stock.v2 else _upcoming_event(stock, report_date)
...
def score_market(
    ...
    stocks = load_stocks(db_path, report_date, market_name, v2_review_path)
...
def main() -> int:
    ...
    for market in ("hk", "us"):
        ...
        results = score_market(args.db, args.date, market, research, args.v2_review)
