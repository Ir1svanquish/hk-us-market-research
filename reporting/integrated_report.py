...
from reporting.hk_key_variables import INTEGRATED_PAGE_CSS, render_integrated_page
from reporting.official_sources.macro_catalog import translate_macro_title
from reporting.official_sources.macro_scenarios import analyze_release
...
def _hk_flow_sentiment(
    ...
    """Replace stale analysis prose with same-day audited HK flow facts."""
    coverage = _hk_pool_coverage(ranked)
    total = coverage["total"]
    flat = total - coverage["advancers"] - coverage["decliners"]
    southbound = float(key_variables["southbound"]["net_hkd_bn"])
...
def render_report(
    *,
    ...
    from reporting.ai_crowding import (
    ...
    if market == "us":
        ...
        history_path = REPORTING_ROOT / "state" / "ai_crowding_history.json"
