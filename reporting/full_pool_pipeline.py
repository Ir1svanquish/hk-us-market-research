"""Build the fully covered stock-pool snapshot for the formal report.

This command prepares report data without sending notifications or editing the
scheduler. Official-source sync remains a separate prerequisite so a slow or
failed refresh can be inspected before scoring starts.
"""
...
from reporting.data_contract import build_pool_contract, write_pool_contract
from reporting.market_foundation import build_relative_strength
from reporting.official_sources.event_context_cli import main as build_event_context
from reporting.official_sources.finnhub import FinnhubClient
from reporting.official_sources.storage import OfficialFilingStore
from reporting.opportunity_scoring import render_markdown, score_market
from reporting.research_report import DEFAULT_DB, load_stocks, parse_market_research
from reporting.top5_validation import (
...
def build_us_snapshots(
    *,
    ...
    client = FinnhubClient(_read_env_keys(env_file, "FINNHUB_API_KEYS"), cache_dir)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the formal report stock-pool contract")
    ...
    parser.add_argument(
        "--official-sync",
        type=Path,
        default=REPORTING_ROOT / "output" / "full_pool_official_sync_20260821.json",
    )
    parser.add_argument(
        "--official-database",
        type=Path,
        default=REPORTING_ROOT / "state" / "official_filings.sqlite3",
    )
    parser.add_argument("--accuracy-ledger", type=Path, default=DEFAULT_LEDGER)
    parser.add_argument("--analysis-top5-state", type=Path, default=DEFAULT_ANALYSIS_STATE)
    ...
    snapshots = (
        build_us_snapshots(
            ...
            cache_dir=REPORTING_ROOT / "cache" / "finnhub",
    ...
    if hk_symbols:
        context_args.extend(
            [
                ...
                "--hk-analysis-report", str(PROJECT_ROOT / "reports" / f"report_{token}_hk.md"),
    ...
    if us_symbols:
        context_args.extend(
            [
                ...
                "--us-analysis-report", str(PROJECT_ROOT / "reports" / f"report_{token}_us.md"),
    ...
    analysis_rows = accuracy_ledger.import_analysis_history(args.analysis_top5_state)
    reporting_rows = accuracy_ledger.record_contract(contract)
