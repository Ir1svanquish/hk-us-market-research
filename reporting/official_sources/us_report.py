"""Generate a five-stock U.S. official-event report."""
...
def render(
    *, symbols: Sequence[str], as_of: date, analysis_report: Path, snapshots: Mapping[str, Mapping[str, object]],
    output_html: Path, output_pdf: Path, snapshot_output: Path, sec_direct: bool = False
) -> Dict[str, object]:
    summary = _parse_summary(analysis_report)
    cards = []
    upcoming = 0
    changed = 0
    filing_count = 0
    for symbol in symbols:
        ...
        <div class="decision-grid"><div><span>分析结论</span><b>{_e(original['decision'])}</b></div>
        <div class="{change_class}"><span>官方事件闸门</span><b>{_e(adjusted)}</b></div></div>
    ...
    doc = _document(
        as_of=as_of,
        analysis_date=analysis_report.stem.replace("report_", "").replace("_us", ""),
        upcoming=upcoming,
        changed=changed,
...
def _document(*, as_of: date, analysis_date: str, upcoming: int, changed: int, filing_count: int, cards: str, sec_direct: bool) -> str:
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>美股官方事件报告</title><style>
...
</style></head><body><div class="cover"><div class="eyebrow">DAILY STOCK ANALYSIS · FORMAL REPORT</div><h1>美股官方事件报告</h1><div class="subtitle">快照日 {_e(as_of)}｜分析报告日 {_e(analysis_date)}｜{_e(source_title)}</div><div class="metrics"><div class="metric"><b>5</b>只股票</div><div class="metric"><b>{upcoming}</b>个财报窗口</div><div class="metric"><b>{changed}</b>项结论降档</div><div class="metric"><b>{filing_count}</b>份 SEC 直连申报</div></div></div>
<div class="note"><b>重要边界：</b>{_e(boundary)}</div>{cards}</body></html>"""


def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate the U.S. official-event report")
    ...
    parser.add_argument("--cache-dir", type=Path, default=REPORTING_ROOT / "cache" / "finnhub")
    parser.add_argument("--database", type=Path, default=REPORTING_ROOT / "state" / "official_filings.sqlite3")
    parser.add_argument("--output-html", type=Path, default=REPORTING_ROOT / "output" / "us_top5_official_events.html")
    parser.add_argument("--output-pdf", type=Path, default=REPORTING_ROOT / "output" / "us_top5_official_events.pdf")
    parser.add_argument("--snapshot-output", type=Path, default=REPORTING_ROOT / "output" / "us_top5_snapshot.json")
    args = parser.parse_args(argv)
    symbols = [item.strip().upper() for item in args.symbols.split(",") if item.strip()]
    client = FinnhubClient(_read_env_keys(args.env_file), args.cache_dir)
    ...
    result = render(symbols=symbols, as_of=args.as_of, analysis_report=args.analysis_report,
