"""Render the HKEX official-event section for the formal report.

The renderer only reads the analysis-stage Markdown report. It never imports
or modifies scheduler, notification, or state modules.
"""
...
def _parse_analysis_summary(path: Path) -> dict[str, dict[str, str]]:
    if not path.exists():
        return {}
...
def render_report(
    *,
    ...
    analysis = _parse_analysis_summary(analysis_report)
        ...
        cards.append(
            ...
            <section class="stock-card">
              ...
              <div class="decision-grid">
                <div><span>分析结论</span><b>{_escape(decision)}</b></div>
                <div class="{change_class}"><span>官方事件闸门</span><b>{_escape(adjusted)}</b></div>
              </div>
...
<html lang="zh-CN"><head><meta charset="utf-8"><title>港股官方事件报告</title>
<style>
...
<div class="cover"><div class="eyebrow">DAILY STOCK ANALYSIS · FORMAL REPORT</div>
<h1>港股官方事件报告</h1><div class="subtitle">报告日 {_escape(as_of.isoformat())}｜头五只港股｜HKEXnews + 董事会业绩日历</div>
...
<div class="note"><b>阅读方法：</b>红色日期框代表港交所董事会日历已确认的日期；“官方事件闸门”用于约束分析阶段的交易动作，不替代行情和技术分析。公告标签可同时出现多个，避免单标签丢失“业绩 + 股息 + 融资”等复合事件。</div>
{cards}
<section class="footer-note"><h2>边界与结论</h2><ul>
<li>本报告汇总官方事件核验结果，供港股正式日报使用。</li>
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate the HK official-event report")
    ...
    parser.add_argument("--output-html", type=Path, default=REPORTING_ROOT / "output" / "hk_official_events.html")
    parser.add_argument("--output-pdf", type=Path, default=REPORTING_ROOT / "output" / "hk_official_events.pdf")
