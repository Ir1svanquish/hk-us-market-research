"""Render the unified-event and macro-scenario report."""
...
def _document(*, as_of: date, totals: Mapping[str, object], material_count: int, gate_cards: str, macro_count: int, selected_count: int, ready_count: int, trade_cards: str, released_cards: str, scenario_cards: str) -> str:
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>统一事件闸门与宏观场景</title><style>
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render the unified event report")
    parser.add_argument("--context-json", type=Path, required=True)
    parser.add_argument("--output-html", type=Path, default=REPORTING_ROOT / "output" / "event_context.html")
    parser.add_argument("--output-pdf", type=Path, default=REPORTING_ROOT / "output" / "event_context.pdf")
