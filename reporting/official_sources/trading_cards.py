"""Deterministic standardized trade cards for the formal report."""
...
def _analysis_levels(
    ...
    return {
        "setup_type": "analysis_plan",
        "entry": entry,
        "stop": stop,
        "target1": target1,
        "target2": target2,
        "target_basis": target_basis,
        "trigger_basis": "分析阶段点位；缺少可验证价格结构，仅作参考",
        "invalidation_basis": "分析阶段风险线与 ATR 缓冲",
...
def _structure_levels(
    ...
    if structure.get("status") != "ok":
        return _analysis_levels(source_entry, source_stop, source_targets, atr, market)
...
def build_trade_card(
    ...
    *,
    ...
    setup_type = str(levels.get("setup_type") or "analysis_plan")
    ...
    if not trigger_condition:
        if entry is None:
            trigger_condition = "分析阶段没有可解析的触发价"
        elif levels.get("setup_type") == "analysis_plan" and any(
