"""CLI for the unified event context and hard-gate engine."""
...
def main(argv: List[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build unified official and macro event context")
    ...
    parser.add_argument("--official-database", type=Path, default=REPORTING_ROOT / "state" / "official_filings.sqlite3")
    parser.add_argument("--event-database", type=Path, default=REPORTING_ROOT / "state" / "unified_events.sqlite3")
    parser.add_argument("--finnhub-snapshot", type=Path)
    parser.add_argument("--hk-analysis-report", type=Path)
    parser.add_argument("--us-analysis-report", type=Path)
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Optional runtime environment source; may be supplied more than once",
    )
    parser.add_argument(
        "--decision-ledger",
        type=Path,
        default=REPORTING_ROOT / "state" / "decision_accuracy.sqlite3",
    )
    parser.add_argument("--cache-dir", type=Path, default=REPORTING_ROOT / "cache" / "bls")
    parser.add_argument("--nasdaq-cache-dir", type=Path, default=REPORTING_ROOT / "cache" / "nasdaq_macro")
    parser.add_argument(
        "--macro-calibration",
        type=Path,
        default=REPORTING_ROOT / "state" / "macro_probability_calibration.json",
    )
    parser.add_argument("--bls-seed", type=Path, default=REPORTING_ROOT / "data" / "bls_schedule_seed_2026.json")
    parser.add_argument("--output-json", type=Path, default=REPORTING_ROOT / "output" / "event_context.json")
    parser.add_argument("--output-markdown", type=Path, default=REPORTING_ROOT / "output" / "event_context.md")
    ...
    report_items = {}
    if args.hk_analysis_report and args.hk_analysis_report.exists():
        report_items.update(parse_analysis_report(args.hk_analysis_report, "hk"))
    if args.us_analysis_report and args.us_analysis_report.exists():
        report_items.update(parse_analysis_report(args.us_analysis_report, "us"))
    coverage = {
