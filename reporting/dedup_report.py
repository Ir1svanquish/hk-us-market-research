"""Render a deduplicated analysis summary for the formal report.

The analysis-stage Markdown remains the source of truth.  This module only parses
that already-produced artifact and creates a concise analysis summary; it does
...
def render_html(report: DedupReport) -> str:
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>{market_label}分析摘要</title><style>
...
<div class="cover"><div class="eyebrow">DAILY STOCK ANALYSIS · ANALYSIS SUMMARY</div><h1>{market_label}研究报告精简版</h1><div class="subtitle">报告日 {_e(report.report_date)}｜仅重排分析阶段已有内容，不重新分析，供正式报告整合</div><div class="metrics"><div class="metric"><b>{len(report.stocks)}</b>分析股票</div><div class="metric"><b>{counts['买入']}</b>买入</div><div class="metric"><b>{counts['持有']}</b>持有</div><div class="metric"><b>{counts['减仓'] + counts['卖出']}</b>减仓/卖出</div><div class="metric"><b>{counts['观望']}</b>观望</div></div></div>
...
<div class="source">来源：{_e(report.source)}。详细分析与原始结论会继续用于生成正式报告。</div>
</body></html>"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render a deduplicated analysis summary")
